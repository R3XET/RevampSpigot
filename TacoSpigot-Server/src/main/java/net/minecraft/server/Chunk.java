...
public class Chunk {
            ...
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    if (this.g[i + j * 16]) {
                        ...
                        //TODO ADDED CODE
                        for (iterator = EnumDirection.EnumDirectionLimit.HORIZONTAL.iterator(); iterator.hasNext(); j1 = FastMath.min(j1, this.world.b(l + enumdirection.getAdjacentX(), i1 + enumdirection.getAdjacentZ()))){
                        //TODO ADDED CODE
    ...
    public IBlockData a(BlockPosition blockposition, IBlockData iblockdata, boolean bool) {
        return a(blockposition, iblockdata, bool);
    }
    ...
    public void a(Entity entity) {
        ...
        */
        ...
        //TODO ADDED CODE
        int k = (int)FastMath.floor(entity.locY / 16.0D);
        //TODO ADDED CODE
    ...
    public void a(Entity entity, AxisAlignedBB axisalignedbb, List<Entity> list, Predicate<? super Entity> predicate) {
        ...
        //TODO ADDED CODE
        int i = (int)FastMath.floor((axisalignedbb.b - 2.0D) / 16.0D);
        int j = (int)FastMath.floor((axisalignedbb.e + 2.0D) / 16.0D);
        //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public void aTNT(Entity entity, AxisAlignedBB axisalignedbb, List<Entity> list) {
        for (int k = MathHelper.clamp(MathHelper.floor((axisalignedbb.b - 2.0) / 16.0), 0, this.entitySlices.length - 1); k <= MathHelper.clamp(MathHelper.floor((axisalignedbb.e + 2.0) / 16.0), 0, this.entitySlices.length - 1); ++k) {
            if (!this.entitySlices[k].isEmpty()) {
                for (Entity entity2 : this.entitySlices[k]) {
                    if (entity2.getBoundingBox().b(axisalignedbb) && entity2 != entity && !entity2.dead && entity2.isTNTorSand) {
                        list.add(entity2);
                    }
                }
            }
    ...
    public void aTNTOnly(Entity entity, AxisAlignedBB axisalignedbb, double x, double y, double z, double ticksLived) {
        for (int k = MathHelper.clamp(MathHelper.floor((axisalignedbb.b - 2.0) / 16.0), 0, this.entitySlices.length - 1); k <= MathHelper.clamp(MathHelper.floor((axisalignedbb.e + 2.0) / 16.0), 0, this.entitySlices.length - 1); ++k) {
            for (Entity entity2 : this.entitySlices[k]) {
                if (!entity2.dead && entity2.locX == x && entity2.locY == y && entity2.locZ == z && entity2.isTNT && entity2.ticksLived == ticksLived && entity2 != entity) {
                    entity2.dead = true;
                    entity2.canExplode = false;
                    entity.multiplier += entity2.multiplier;
                }
            }
        }
    }
    ...
    public <T extends Entity> void a(Class<? extends T> oclass, AxisAlignedBB axisalignedbb, List<T> list, Predicate<? super T> predicate) {
        ...
        //TODO ADDED CODE
        int counts[], i = (int)FastMath.floor((axisalignedbb.b - 2.0D) / 16.0D);
        int j = (int)FastMath.floor((axisalignedbb.e + 2.0D) / 16.0D);
        //TODO ADDED CODE
