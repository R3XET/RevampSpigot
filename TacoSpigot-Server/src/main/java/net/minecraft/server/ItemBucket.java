        ...
        if (movingobjectposition == null) {
            ...
            if (movingobjectposition.type == MovingObjectPosition.EnumMovingObjectType.BLOCK) {
                ...
                if (flag) {
                    ...
                    if (Settings.IMP.SETTINGS.ENTITY.SPAWNERS_ACTS_AS_SPONGES) {
                        EnumDirection[] var10;
                        int var11 = (var10 = EnumDirection.values()).length;
                        for (int var9 = 0; var9 < var11; var9++) {
                            EnumDirection direction = var10[var9];
                            Block block = world.getType(blockposition1.shift(direction)).getBlock();
                            if (block instanceof BlockMobSpawner)
                                return itemstack;
                        }
                    }
                    if (!Settings.IMP.SETTINGS.PERFORMANCE.TNT.WATER_BREAKS_REDSTONE_ITEMS) {
                        Block block = world.getType(blockposition1).getBlock();
                        if (block instanceof BlockRedstoneWire || block instanceof BlockRedstoneTorch || block instanceof BlockDiodeAbstract || block instanceof BlockButtonAbstract)
                            return itemstack;
                    }
