...
public class BlockFlowing extends BlockFluids {
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        ...
        if (i > 0) {
            ...
            //TODO ADDED CODE
            if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.INFINITE_WATER_SOURCES && this.a >= 2 && this.material == Material.WATER) {
                IBlockData iblockdata1 = world.getType(blockposition.down());
                if (iblockdata1.getBlock().getMaterial().isBuildable()) {
                    i1 = 0;
                } else if (iblockdata1.getBlock().getMaterial() == this.material && iblockdata1.<Integer>get(LEVEL) == 0) {
                    i1 = 0;
                }
            }
            //TODO ADDED CODE
        ...
        if (this.h(world, blockposition.down(), iblockdata2)) {
            ...
            if (!event.isCancelled()) {
                if (this.material == Material.LAVA && world.getType(blockposition.down()).getBlock().getMaterial() == Material.WATER && Settings.IMP.SETTINGS.PERFORMANCE.LAVA_TO_COBBLE) {
            ...
            */
            ...
            //TODO ADDED CODE
            if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.BETTER_SPONGE_MECHANICS && !world.canFlow(blockposition.down())) return;
            //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public static boolean g(World world, SmallCachedRegion region, BlockPosition blockposition, Material material) {
        return !((!Settings.IMP.SETTINGS.PERFORMANCE.TNT.BETTER_SPONGE_MECHANICS || world.canFlow(blockposition)) && !gOld(region, blockposition, material));
    }

    private static boolean gOld(SmallCachedRegion region, BlockPosition blockposition, Material material) {
        Block block = region.getBlock(blockposition);
        return !(block != Blocks.STANDING_SIGN &&
                ...
                Settings.IMP.SETTINGS.PERFORMANCE.TNT.BLOCK_GENERATION_FROM_LIQUIDS || !block.material.isLiquid() || material == block.material) && (
                Settings.IMP.SETTINGS.PERFORMANCE.TNT.WATER_BREAKS_REDSTONE_ITEMS || (!(block instanceof BlockRedstoneWire) && !(block instanceof BlockRedstoneTorch) && !(block instanceof BlockDiodeAbstract) && !(block instanceof BlockButtonAbstract))));
    }
