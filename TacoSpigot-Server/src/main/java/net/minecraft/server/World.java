...
import eu.revamp.spigot.optimization.cache.SmallCachedRegion;
...
public abstract class World implements IBlockAccess {
    ...
    public final Map<MovingCannoningEntity, MovementData> movementCache = new HashMap<>();
    public final Map<ObjectLocation, List<EntityTNTPrimed>> tntMovementCache = new HashMap<>();

    public final Set<ObjectLocation> explosionPacketCache = new HashSet<>();
    //TODO ADDED CODE

    //TODO ADDED CODE
    public static class MovingCannoningEntity {
        ...
        public MovingCannoningEntity(Entity entity) {
            ...
            this.explosion = entity.world.explosion;
            this.hash = Double.doubleToLongBits(this.locX);
            this.hash = 17L * this.hash + Double.doubleToLongBits(this.locY);
            this.hash = 17L * this.hash + Double.doubleToLongBits(this.locZ);
            this.hash = 17L * this.hash + Double.doubleToLongBits(this.motX);
            this.hash = 17L * this.hash + Double.doubleToLongBits(this.motY);
            this.hash = 17L * this.hash + Double.doubleToLongBits(this.motZ);
            this.hash = 17L * this.hash + this.explosion;
        }

        public boolean equals(Object object) {
            MovingCannoningEntity entity = (MovingCannoningEntity)object;
            return (entity.locX == this.locX && entity.locY == this.locY && entity.locZ == this.locZ && entity.motX == this.motX && entity.motY == this.motY && entity.motZ == this.motZ && entity.explosion == this.explosion);
        }
    ...
    public static class MovementData {
        ...
        public MovementData(double[] doubles, boolean[] booleans) {
    ...
    public boolean canFlow(BlockPosition blockPosition) {
        Integer integer = getSpongeCache().get(blockPosition);
        return !(integer != null && integer > 0);
    }


    public Map<BlockPosition, Integer> getSpongeCache() {
        return this.worldData.spongeLocationCache;
    }

    public void sponge(BlockPosition blockPosition) {
        for (int x = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; x <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; x++) {
            for (int y = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; y <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; y++) {
                for (int z = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; z <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; z++) {
                    BlockPosition position = blockPosition.a(x, y, z);
                    if (getType(position).getBlock().getMaterial().isLiquid()) {
                        setTypeAndData(position, Blocks.AIR.getBlockData(), 2);
                        applyPhysics(position, Blocks.AIR);
                    }
                    Integer integer = getSpongeCache().get(position);
                    getSpongeCache().put(position, (integer == null) ? 1 : (integer + 1));
                }
            }
        }
    }

    public void removeSpongeLocation(BlockPosition blockPosition) {
        for (int x = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; x <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; x++) {
            for (int y = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; y <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; y++) {
                for (int z = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; z <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; z++) {
                    BlockPosition position = blockPosition.a(x, y, z);
                    ...
                    integer = integer - 1;
                    if (integer < 1) {
                        getSpongeCache().remove(position);
                    } else {
                        getSpongeCache().put(position, integer);
                    }
                }
            }
        }
    ...
    public static class ObjectLocation {
        ...
        public ObjectLocation(double locX, double locY, double locZ) {
            this.locX = locX;
            this.locY = locY;
            this.locZ = locZ;
            long hash = Double.doubleToLongBits(locX);
            hash = 31L * hash + Double.doubleToLongBits(locY);
            hash = 31L * hash + Double.doubleToLongBits(locZ);
            this.hash = (int)hash;
        }

        public boolean equals(Object object) {
            ObjectLocation tnt = (ObjectLocation)object;
            return (tnt.locX == this.locX && tnt.locY == this.locY && tnt.locZ == this.locZ);
        }
    ...
    public /* bridge */ boolean setTypeAndDataNoLightUpdate(BlockPosition blockposition, IBlockData iblockdata, int i) {
        if (this.captureTreeGeneration) {
            BlockState blockstate = null;
        ...
        if (this.captureBlockStates) {
            blockstate2 = CraftBlockState.getBlockState(this, blockposition.getX(), blockposition.getY(), blockposition.getZ(), i);
            this.capturedBlockStates.add(blockstate2);
        }
        IBlockData iblockdata2 = chunk.a(blockposition, iblockdata, false);
        if (iblockdata2 == null) {
            if (this.captureBlockStates) {
        ...
        if (!this.captureBlockStates) {
            this.notifyAndUpdatePhysics(blockposition, chunk, block2, block, i);
        }
        return true;
    }


    public boolean setTypeAndDataNoLightUpdateSand(BlockPosition blockposition, IBlockData iblockdata, int i) {
        if (this.captureTreeGeneration) {
            BlockState blockstate = null;
            Iterator<BlockState> it = this.capturedBlockStates.iterator();
            while (it.hasNext()) {
        ...
        Block block2 = iblockdata2.getBlock();
        if (!this.captureBlockStates) {
            this.notifyAndUpdatePhysicsSand(blockposition, chunk, block2, block, i);
        }
        return true;
    }
    ...
    public boolean setAirNoLightUpdate(net.minecraft.server.BlockPosition a0) {
        return this.setTypeAndDataNoLightUpdate(a0, net.minecraft.server.Blocks.AIR.getBlockData(), 3);
    }

    public void notifyAndUpdatePhysicsSand(BlockPosition blockposition, Chunk chunk, Block oldBlock, Block newBLock, int flag) {
        if ((flag & 2) != 0x0 && (chunk == null || chunk.isReady())) {
            this.notify(blockposition);
        }
        if (!this.isClientSide && (flag & 1) != 0x0) {
            this.updateSand(blockposition, oldBlock);
            if (newBLock.isComplexRedstone()) {
                this.updateAdjacentComparators(blockposition, newBLock);
            }
        }
    ...
    public void updateSand(BlockPosition blockposition, Block block) {
        if (this.worldData.getType() != WorldType.DEBUG_ALL_BLOCK_STATES) {
            if (this.populating) {
                return;
            }
            this.applyPhysicsSand(blockposition, block);
        }
    }

    public void applyPhysicsSand(BlockPosition blockposition, Block block) {
    ...
    public int k(BlockPosition blockposition) {
        if (blockposition.getY() < 0) {
            ...
            //TODO ADDED CODE
            if (!Settings.IMP.SETTINGS.PERFORMANCE.LOAD_CHUNKS_FOR_LIGHT_CHECKS && !isLoaded(blockposition))
                return 0;
            //TODO ADDED CODE
        ...
        if (!Double.isNaN(vec3d.a) && !Double.isNaN(vec3d.b) && !Double.isNaN(vec3d.c)) {
            if (!Double.isNaN(vec3d1.a) && !Double.isNaN(vec3d1.b) && !Double.isNaN(vec3d1.c)) {
                ...
                */
                ...
                //TODO ADDED CODE
                int i = (int)FastMath.floor(vec3d1.a);
                int j = (int)FastMath.floor(vec3d1.b);
                int k = (int)FastMath.floor(vec3d1.c);
                int l = (int)FastMath.floor(vec3d.a);
                int i1 = (int)FastMath.floor(vec3d.b);
                int j1 = (int)FastMath.floor(vec3d.c);
                //TODO ADDED CODE
                ...
                while (k1-- >= 0) {
                    ...
                    */
                    ...
                    //TODO ADDED CODE
                    l = (int)FastMath.floor(vec3d.a) - ((enumdirection == EnumDirection.EAST) ? 1 : 0);
                    i1 = (int)FastMath.floor(vec3d.b) - ((enumdirection == EnumDirection.UP) ? 1 : 0);
                    j1 = (int)FastMath.floor(vec3d.c) - ((enumdirection == EnumDirection.SOUTH) ? 1 : 0);
                    //TODO ADDED CODE
    ...
    public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed signature, added SpawnReason
        ...
        //TODO ADDED CODE
        int i = (int)FastMath.floor(entity.locX / 16.0D);
        int j = (int)FastMath.floor(entity.locZ / 16.0D);
        //TODO ADDED CODE
...
/*
        //TODO ADDED CODE
        int i = (int)FastMath.floor(axisalignedbb.a);
        int j = (int)FastMath.floor(axisalignedbb.d + 1.0D);
        int k = (int)FastMath.floor(axisalignedbb.b);
        int l = (int)FastMath.floor(axisalignedbb.e + 1.0D);
        int i1 = (int)FastMath.floor(axisalignedbb.c);
        int j1 = (int)FastMath.floor(axisalignedbb.f + 1.0D);
        //TODO ADDED CODE

        WorldBorder worldborder = getWorldBorder();
        boolean flag = entity.aT();
        boolean flag1 = a(worldborder, entity);
        boolean special = !(!(entity instanceof EntityTNTPrimed) && !(entity instanceof EntityFallingBlock));
        if (!flag1 && special && Settings.IMP.SETTINGS.PERFORMANCE.TNT.DESTROY_TNT_AND_FALLING_BLOCKS_OUTSIDE_BORDER) {
            entity.die();
            return arraylist;
        }
        int ystart = (k - 1 < 0) ? 0 : (k - 1);
                ...
                for (int x = xstart; x < xend; x++) {
                    for (int z = zstart; z < zend; z++) {
                        for (int y = ystart; y < l; y++) {
                            ...
                            if (!getWorldBorder().a(blockposition) && flag1 && !special) {
                                block = Blocks.STONE.getBlockData();
                            } else {
                                try{
                                    block = chunk.getBlockData(blockposition);
                            ...
                            if (block != null) {
                                Block b = block.getBlock();
                                if (entity.world.paperSpigotConfig.fallingBlocksCollideWithSigns && special && (b instanceof BlockSign || b instanceof BlockSkull || b instanceof BlockFenceGate || b instanceof BlockTorch || b instanceof BlockButtonAbstract || b instanceof BlockLever || b instanceof BlockTripwireHook || b instanceof BlockTripwire || b instanceof BlockChest || b instanceof BlockSlowSand || b instanceof BlockBed || b instanceof BlockEnderChest || b instanceof BlockEnchantmentTable || b instanceof BlockBrewingStand)) {
        ...
        if (special || entity instanceof EntityItem || entity instanceof EntityArmorStand)
            return arraylist;
        double d0 = 0.25D;
        List<Entity> list = getEntities(entity, axisalignedbb.grow(d0, d0, d0));
        for (int j2 = 0; j2 < list.size(); j2++) {
            if (entity.passenger != list && entity.vehicle != list) {
...
*/
    ...
    public List<AxisAlignedBB> getCubes(Entity entity, AxisAlignedBB axisalignedbb) {
        ...
        int k = (int)FastMath.floor(axisalignedbb.b);
        int l = (int)FastMath.floor(axisalignedbb.e + 1.0D);
        int i2 = (int)FastMath.floor(axisalignedbb.c);
        int j2 = (int)FastMath.floor(axisalignedbb.f + 1.0D);
        WorldBorder worldborder = this.getWorldBorder();
        boolean flag = entity.aT();
        boolean flag2 = this.a(worldborder, entity);
        boolean special = entity instanceof EntityTNTPrimed || entity instanceof EntityFallingBlock;
        if (!flag2 && special && Settings.IMP.SETTINGS.PERFORMANCE.TNT.DESTROY_TNT_AND_FALLING_BLOCKS_OUTSIDE_BORDER) {
            entity.die();
            return arraylist;
        }
        int ystart = (k - 1 < 0) ? 0 : (k - 1);
        for (int chunkx = i >> 4; chunkx <= j - 1 >> 4; ++chunkx) {
            ...
            for (int chunkz = i2 >> 4; chunkz <= j2 - 1 >> 4; ++chunkz) {
                ...
                int xstart = (i < cx) ? cx : i;
                int xend = (j < cx + 16) ? j : (cx + 16);
                int zstart = (i2 < cz) ? cz : i2;
                int zend = (j2 < cz + 16) ? j2 : (cz + 16);
                for (int x = xstart; x < xend; ++x) {
                    for (int z = zstart; z < zend; ++z) {
                        for (int y = ystart; y < l; ++y) {
                            BlockPosition blockposition = new BlockPosition(x, y, z);
                            ...
                            IBlockData block;
                            if (!this.getWorldBorder().a(blockposition) && flag2 && !special) {
                                block = Blocks.STONE.getBlockData();
                            }
                            else {
                                block = chunk.getBlockData(blockposition);
        ...
        for (int j3 = 0; j3 < list.size(); ++j3) {
            if (entity.passenger != list && entity.vehicle != list) {
                AxisAlignedBB axisalignedbb2 = ((Entity)list.get(j2)).S();
                if (axisalignedbb2 != null && axisalignedbb2.b(axisalignedbb)) {
                    arraylist.add(axisalignedbb2);
                }
                axisalignedbb2 = entity.j((Entity)list.get(j3));
                if (axisalignedbb2 != null && axisalignedbb2.b(axisalignedbb)) {
                    arraylist.add(axisalignedbb2);
                }
            }
    ...
    public void checkLocation(boolean flag, boolean flag1, boolean special, Entity entity, SmallCachedRegion region, int x, int y, int z, List<AxisAlignedBB> list, AxisAlignedBB axisalignedbb) {
        IBlockData block;
        BlockPosition blockposition = new BlockPosition(x, y, z);
        if (flag && flag1) {
            entity.h(false);
        ...
        if (!getWorldBorder().a(blockposition) && flag1 && !special) {
            ...
            block = region.getType(blockposition);
        }
        if (block != null) {
            Block b = block.getBlock();
            if (entity.world.paperSpigotConfig.fallingBlocksCollideWithSigns && special && (b instanceof BlockSign || b instanceof BlockSkull || b instanceof BlockFenceGate || b instanceof BlockTorch || b instanceof BlockButtonAbstract || b instanceof BlockLever || b instanceof BlockTripwireHook || b instanceof BlockTripwire || b instanceof BlockChest || b instanceof BlockSlowSand || b instanceof BlockBed || b instanceof BlockEnderChest || b instanceof BlockEnchantmentTable || b instanceof BlockBrewingStand)) {
                AxisAlignedBB bb = AxisAlignedBB.a(x, y, z, x + 1.0D, y + 1.0D, z + 1.0D);
                if (axisalignedbb.b(bb))
                    list.add(bb);
            } else {
                b.a(this, blockposition, block, axisalignedbb, list, entity);
            }
        }
    }
    //TODO ADDED CODE
    ...
    public int a(float f) {
        ...
        //TODO ADDED CODE
        float f2 = 1.0F - (float)FastMath.cos((f1 * 3.1415927F * 2.0F)) * 2.0F + 0.5F;
        //TODO ADDED CODE
    ...
    public void tickEntities() {
        ...
        //TODO ADDED CODE
        if (MinecraftServer.currentTick % Settings.IMP.SETTINGS.ENTITY.ENTITY_ACTIVATION_RANGE_DELAY == 0)
            org.spigotmc.ActivationRange.activateEntities(this);
        //TODO ADDED CODE

        //TODO ADDED CODE
        for (Entity entity2 : this.entityList) {
            if (entity2.isTNT && entity2.isAlive()) {
                this.getChunkAt(entity2.getChunkX(), entity2.getChunkZ()).aTNTOnly(entity2, entity2.getBoundingBox(), entity2.locX, entity2.locY, entity2.locZ, entity2.ticksLived);
            }
        }
        //TODO ADDED CODE
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        ...
        //TODO ADDED CODE
        int i = (int)FastMath.floor(entity.locX);
        int j = (int)FastMath.floor(entity.locZ);
        //TODO ADDED CODE
        byte b0 = 32;
        ...
        if (!org.spigotmc.ActivationRange.checkIfActive(entity)) {
            ...
            //TODO ADDED CODE
            int k = (int)FastMath.floor(entity.locX / 16.0D);
            int l = (int)FastMath.floor(entity.locY / 16.0D);
            int i1 = (int)FastMath.floor(entity.locZ / 16.0D);
            //TODO ADDED CODE
    ...
    public boolean c(AxisAlignedBB axisalignedbb) {
        ...
        */
        ...
        //TODO ADDED CODE
        int i = (int)FastMath.floor(axisalignedbb.a);
        int j = (int)FastMath.floor(axisalignedbb.d);
        int k = (int)FastMath.floor(axisalignedbb.b);
        int l = (int)FastMath.floor(axisalignedbb.e);
        int i1 = (int)FastMath.floor(axisalignedbb.c);
        int j1 = (int)FastMath.floor(axisalignedbb.f);
        //TODO ADDED CODE
    ...
    protected void D() {
        ...
        for (i = 0; i < this.players.size(); ++i) {
            ...
            //TODO ADDED CODE
            j = (int)FastMath.floor(entityhuman.locX / 16.0D);
            k = (int)FastMath.floor(entityhuman.locZ / 16.0D);
            //TODO ADDED CODE
            l = this.q();
        ...
        if (spigotConfig.randomLightUpdates && !this.players.isEmpty()) { // Spigot
            ...
            //TODO ADDED CODE
            j = (int)FastMath.floor(entityhuman.locX) + this.random.nextInt(11) - 5;
            k = (int)FastMath.floor(entityhuman.locY) + this.random.nextInt(11) - 5;
            l = (int)FastMath.floor(entityhuman.locZ) + this.random.nextInt(11) - 5;
            //TODO ADDED CODE
    ...
    public List<Entity> a(Entity entity, AxisAlignedBB axisalignedbb, Predicate<? super Entity> predicate) {
        ...
        //TODO OLD CODE
        //TODO ADDED CODE
        int i = (int)FastMath.floor((axisalignedbb.a - 2.0D) / 16.0D);
        int j = (int)FastMath.floor((axisalignedbb.d + 2.0D) / 16.0D);
        int k = (int)FastMath.floor((axisalignedbb.c - 2.0D) / 16.0D);
        int l = (int)FastMath.floor((axisalignedbb.f + 2.0D) / 16.0D);
        //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public List<Entity> aTNT(Entity entity, AxisAlignedBB axisalignedbb) {
        List<Entity> arrayList = new ArrayList<>();
        for (int i1 = MathHelper.floor((axisalignedbb.a - 2.0) / 16.0); i1 <= MathHelper.floor((axisalignedbb.d + 2.0) / 16.0); ++i1) {
            for (int j1 = MathHelper.floor((axisalignedbb.c - 2.0) / 16.0); j1 <= MathHelper.floor((axisalignedbb.f + 2.0) / 16.0); ++j1) {
                if (this.isChunkLoaded(i1, j1, true)) {
                    this.getChunkAt(i1, j1).aTNT(entity, axisalignedbb, arrayList);
                }
            }
        }
        return arrayList;
    ...
    public int a(Class<?> oclass) {
        int i = 0;
        Iterator iterator = Settings.IMP.SETTINGS.ASYNC.CONCURRENT_WORLD_SUPPORT ? (
                new ArrayList<>((Collection)this.entityList)).iterator() :
                this.entityList.iterator();
        while (iterator.hasNext()) {
