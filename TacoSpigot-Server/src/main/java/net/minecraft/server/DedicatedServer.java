...
import eu.revamp.spigot.events.ArmorEvents;
import eu.revamp.spigot.optimization.TNTOptimization;
import eu.revamp.spigot.optimization.cache.RaidingPatches;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
    ...
    protected boolean init() throws IOException {
        ...
        if (!org.spigotmc.SpigotConfig.lateBind) {
            ...
            if (!NameReferencingFileConverter.a(this.propertyManager)) {
                ...
                //TODO ADDED CODE
        for (int id = 0; id < 453; ++id) {
            if (Block.getById(id) != null) {
                Block b = Block.getById(id);
                if (b.durability == 0.0f && b.getMaterial() != Material.AIR && b.getMaterial() != Material.FIRE) {
                    b.durability = 0.1f;
                }
            }
        }
        /*
        if (RevampSpigotConfig.obsidianDestoyer) {
            Bukkit.getLogger().info("Starting RevampSpigot Obsidian Destroyer");
            for (int id2 = 0; id2 < 453; ++id2) {
                if (Block.getById(id2) != null) {
                    Block b2 = Block.getById(id2);
                    if (b2.durability == 3000.0f || b2.durability == 6000.0f || b2.durability == 1.8E7f) {
                        if (id2 == 49) {
                            b2.durability = 7001.0f;
                        }
                        else if (id2 == 130) {
                            b2.durability = 7002.0f;
                        }
                        else if (id2 == 116) {
                            b2.durability = 7003.0f;
                        }
                        else if (id2 == 145) {
                            b2.durability = 7004.0f;
                        }
                        else if (id2 == 120) {
                            b2.durability = 7005.0f;
                        }
                        else if (id2 == 7) {
                            b2.durability = 7006.0f;
                        }
                    }
                }
            }
        ...
        */
        Bukkit.getLogger().info("Starting RevampSpigot TNT Optimizations");
        boolean combineTNT = Settings.IMP.SETTINGS.PERFORMANCE.TNT.COMBINE_TNT;
        Bukkit.getLogger().info("--------------------------------------------");
        Bukkit.getLogger().info("");
        Bukkit.getLogger().info("STARTING RevampSpigot TNT CALCULATIONS...");
        Bukkit.getLogger().info("");
        Bukkit.getLogger().info("-------------------------------------------");
        for (int ex = 0; ex <= 1330; ++ex) {
            TNTOptimization.example.add(false);
        }
        double d0 = 0.3378378334839559;
        for (float f = 0.0f; f <= 1.0f; f += (float)d0) {
            for (float f2 = 0.0f; f2 <= 1.0f; f2 += (float)d0) {
                for (float f3 = 0.0f; f3 <= 1.0f; f3 += (float)d0) {
                    TNTOptimization.a2s.add(new org.bukkit.util.Vector(f, f2, f3));
                }
            }
        }
        int progress = 0;
        for (int xI = -80; xI <= 80; ++xI) {
            for (int yI = -80; yI <= 80; ++yI) {
                for (int zI = -80; zI <= 80; ++zI) {
                    double x = xI / 10.0;
                    double y = yI / 10.0;
                    double z = zI / 10.0;
                    if (++progress % 417328 == 0) {
                        System.out.println("Progress [" + (int)(progress / 4173280.0 * 10.0) + "/10]");
                    }
                    double d4 = MathHelper.sqrt(x * x + y * y + z * z) / 8.0;
                    if (d4 <= 1.0) {
                        double d5 = x;
                        double d6 = y + 0.49;
                        double d7 = z;
                        ...
                        if (d8 != 0.0) {
                            ...
                            d6 /= d8;
                            d7 /= d8;
                            TNTOptimization.velocities.add(new Vector(d5, d6, d7));
                        }
                        else {
                            TNTOptimization.velocities.add(new Vector(0, 0, 0));
                        }
                    }
                    else {
                        TNTOptimization.velocities.add(new Vector(0, 0, 0));
                    }
                    TNTOptimization.d7s.add(d4);
                }
            }
        }
        Bukkit.getLogger().info("--------------------------------------------");
        ...
        Bukkit.getLogger().info("(Players may now join)");
        Bukkit.getLogger().info("");
        Bukkit.getLogger().info("-------------------------------------------");

        Bukkit.getLogger().info("RevampSpigot TNT Optimizations Completed");

                //DedicatedServer.playersCanJoin = true;
       //TODO SPONGE OPTIMIZE
                /*
        for (int x2 = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; x2 <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; ++x2) {
            for (int y2 = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; y2 <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; ++y2) {
                for (int z2 = -Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; z2 <= Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS; ++z2) {
                    if (Math.abs(x2) == Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS || Math.abs(y2) == Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS || Math.abs(z2) == Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS) {
                        TNTOptimization.spongeValues.add(new CoordinateInt(x2, y2, z2));
                    }
                }
            }
        }
        ...
        /*
        if (RevampSpigotConfig.tpsGraph) {
            new TPSTracker().trackTPS();
            Bukkit.getLogger().info("Started RevampSpigot TPS Tracker");
        }*/
        RaidingPatches rp = new RaidingPatches();
        if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.DISABLE_FENCEGATES_SAND_PLATES) {
            rp.fixSandPlate();
        }
        if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.DISABLE_ROOF_CANNONS) {
            rp.fixRoofCannons();
        }

        Bukkit.getLogger().info("Configured Armor Events");
        new ArmorEvents();
                //TODO ADDED CODE
    ...
    private static void add(int i, String s, Block block) {
        ...
        for (IBlockData iblockdata : block.P().a()) {
            int k = Block.REGISTRY.b(block) << 4 | block.toLegacyData(iblockdata);
