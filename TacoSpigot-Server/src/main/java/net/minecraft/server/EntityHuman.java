...
public abstract class EntityHuman extends EntityLiving {
	...
	private final KnockbackManager KB_MANAGER = RevampSpigot.getInstance().getKnockbackManager();
	...
	public EntityHuman(World world, GameProfile gameprofile) {
		...
		this.setPositionRotation((double) blockposition.getX() + 0.5D, blockposition.getY() + 1,
	...
	public void b(Entity entity, int i) {
		...
		Collection<ScoreboardScore> collection = this.world.getServer().getScoreboardManager()
				.getScoreboardScores(IScoreboardCriteria.f, this.getName(), new java.util.ArrayList<ScoreboardScore>());
	...
	public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
		if (itemstack == null) {
			...
			if (flag) {
				...
				f = 0.3F;
				entityitem.motX = -MathHelper.sin(this.yaw / 180.0F * 3.1415927F)
						* MathHelper.cos(this.pitch / 180.0F * 3.1415927F) * f;
				entityitem.motZ = MathHelper.cos(this.yaw / 180.0F * 3.1415927F)
						* MathHelper.cos(this.pitch / 180.0F * 3.1415927F) * f;
	...
	public void a(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKeyOfType("SpawnX", 99) && nbttagcompound.hasKeyOfType("SpawnY", 99)
				&& nbttagcompound.hasKeyOfType("SpawnZ", 99)) {
			this.c = new BlockPosition(nbttagcompound.getInt("SpawnX"), nbttagcompound.getInt("SpawnY"),
	...
	public boolean a(EntityHuman entityhuman) {
		// CraftBukkit start - Change to check OTHER player's scoreboard team according
		// to API
	...
	private RevampSpigot e1 = RevampSpigot.getInstance();
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (flag2) {
						if (i > 0) {
							...
							//TODO OLD CODE
							//entity.g(-MathHelper.sin(this.yaw * 3.1415927F / 180.0F) * i * 0.5F * 0.95D, 0.0D, MathHelper.cos(this.yaw * 3.1415927F / 180.0F) * i * 0.5F * 0.95D);
							//TODO OLD CODE

							//TODO ADDED CODE
							entity.g(-FastMath.sin((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D, 0.1D, FastMath.cos((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D);
							//TODO ADDED CODE

							this.motX *= 0.6D;
							this.motZ *= 0.6D;
							setSprinting(e1.set_sprinting);
						}
						if (((entity instanceof EntityPlayer)) && (entity.velocityChanged)) {
							double horizontal = entity.getBukkitEntity().isOnGround()
									? e1.getGroundKnockback().getHorizontal()
									: e1.getAirKnockback().getHorizontal();
							double vertical = entity.getBukkitEntity().isOnGround() ? e1.getGroundKnockback().getVertical()
									: e1.getAirKnockback().getVertical();

							if (e1.kb_friction) {
								entity.motX *= (1/e1.kb_frictionValue) + horizontal;
								entity.motY *= (1/e1.kb_frictionValue) + vertical;
								entity.motZ *= (1/e1.kb_frictionValue) + horizontal;
							} else {
								entity.motX *= horizontal;
								entity.motY *= vertical;
								entity.motZ *= horizontal;
							}

							if (this.e1.limit) {
								if (entity.motX > this.e1.horiLimit) {
									entity.motX = (this.e1.horiLimit);
								}
								if (entity.motZ > this.e1.horiLimit) {
									entity.motZ = (this.e1.horiLimit);
								}
								if (entity.motY > this.e1.vertLimit) {
									entity.motY = (this.e1.vertLimit);
								}
							}
	...
	public void attack(Entity entity) {
		if (!Settings.IMP.SETTINGS.KNOCKBACK_PROFILES_ENABLED) {
			if (entity.aD() && !entity.l(this)) {
				...
				float f2 = 0.0F;
				if (entity instanceof EntityLiving) {
					f2 = EnchantmentManager.a(bA(), ((EntityLiving)entity).getMonsterType());
				} else {
					f2 = EnchantmentManager.a(bA(), EnumMonsterType.UNDEFINED);
				}
				int i = 0 + EnchantmentManager.a(this);
				if (isSprinting())
					i++;
				if (f > 0.0F || f2 > 0.0F) {
					...
					boolean flag3 = entity.damageEntity(DamageSource.playerAttack(this), f);
					if (flag3) {
						if (i > 0) {
							entity.g(-FastMath.sin((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D, 0.1D, FastMath.cos((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D);
							this.motX *= 0.6D;
						...
						if (entity instanceof EntityPlayer && entity.velocityChanged) {
							...
							Vector velocity = new Vector(d0, d2, d3);
						...
						if (entity instanceof EntityLiving) {
							...
							if (j > 0) {
								EntityCombustByEntityEvent combustEvent2 = new EntityCombustByEntityEvent(getBukkitEntity(), entity.getBukkitEntity(), j * 4);
								Bukkit.getPluginManager().callEvent(combustEvent2);
								if (!combustEvent2.isCancelled())
									entity.setOnFire(combustEvent2.getDuration());
							}
						}
		...
		if (entity.aD() && !entity.l(this)) {
			...
			if (f > 0.0F || f2 > 0.0F) {
				...
				boolean flag3 = entity.damageEntity(DamageSource.playerAttack(this), f);
				if (flag3) {
					Knockback profile = (getKnockback() == null) ? KB_MANAGER.getDefaultKnockback() : getKnockback();
					if (profile != null) {
						if (isApplyingSprintKnockback()) {
							entity.g((-((float)FastMath.sin((this.yaw * 3.1415927F / 180.0F))) * i * 0.5F) * profile.getSprintHorizontal(), 0.0D, FastMath.cos((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D * profile.getSprintHorizontal());
							this.motY *= profile.getSprintVertical();
							this.motX *= profile.getSlowDown();
							this.motZ *= profile.getSlowDown();
							if (profile.isStopSprint())
								setSprinting(false);
						} else {
							entity.g((-((float)FastMath.sin((this.yaw * 3.1415927F / 180.0F))) * i * 0.5F) * 0.95D, 0.0D, FastMath.cos((this.yaw * 3.1415927F / 180.0F)) * i * 0.5D * 0.95D);
						}
						setApplyingSprintKnockback(false);
						double victimMotX = entity.motX;
						double victimMotY = entity.motY;
						double victimMotZ = entity.motZ;
						if (entity instanceof EntityPlayer) {
							Knockback profile2 = (getKnockback() == null) ? KB_MANAGER.getDefaultKnockback() : getKnockback();
							entity.motX *= this.onGround ? profile2.getHorizontal() : profile2.getAirHorizontal();
							entity.motY *= this.onGround ? profile2.getVertical() : profile2.getAirVertical();
							entity.motZ *= this.onGround ? profile2.getHorizontal() : profile2.getAirHorizontal();
						}
						if (entity instanceof EntityPlayer && entity.velocityChanged) {
							EntityPlayer attackedPlayer = (EntityPlayer)entity;
					...
					if (entity instanceof EntityLiving) {
						a(StatisticList.w, FastMath.round(f * 10.0F));
						if (j > 0) {
							EntityCombustByEntityEvent combustEvent2 = new EntityCombustByEntityEvent(getBukkitEntity(), entity.getBukkitEntity(), j * 4);
							Bukkit.getPluginManager().callEvent(combustEvent2);
							if (!combustEvent2.isCancelled())
	...
	public EntityHuman.EnumBedResult a(BlockPosition blockposition) {
		if (!this.world.isClientSide) {
			...
			if (Math.abs(this.locX - (double) blockposition.getX()) > 3.0D
					|| Math.abs(this.locY - (double) blockposition.getY()) > 2.0D
					|| Math.abs(this.locZ - (double) blockposition.getZ()) > 3.0D) {
			...
			List list = this.world.a(EntityMonster.class,
					new AxisAlignedBB((double) blockposition.getX() - d0, (double) blockposition.getY() - d1,
							(double) blockposition.getZ() - d0, (double) blockposition.getX() + d0,
							(double) blockposition.getY() + d1, (double) blockposition.getZ() + d0));
		...
		if (this.getBukkitEntity() instanceof Player) {
			Player player = (Player) this.getBukkitEntity();
			org.bukkit.block.Block bed = this.world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(),
					blockposition.getZ());
		...
		if (this.world.isLoaded(blockposition)) {
			...
			this.setPosition((float) blockposition.getX() + f,
					(float) blockposition.getY() + 0.6875F, (float) blockposition.getZ() + f1);
		} else {
			this.setPosition((float) blockposition.getX() + 0.5F,
					(float) blockposition.getY() + 0.6875F, (float) blockposition.getZ() + 0.5F);
		}
	...
	public void a(boolean flag, boolean flag1, boolean flag2) {
		...
		if (this.bx != null && iblockdata.getBlock() == Blocks.BED) {
			...
			this.setPosition((float) blockposition.getX() + 0.5F,
					(float) blockposition.getY() + 0.1F, (float) blockposition.getZ() + 0.5F);
		}
		...
		if (this.getBukkitEntity() instanceof Player) {
			...
			if (blockposition != null) {
				bed = this.world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(),
						blockposition.getZ());
			} else {
		...
		if (this.vehicle == null) {
			...
			if (this.a(Material.WATER)) {
				...
				if (i > 0) {
					...
					this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * (float) i * 0.01F); // PaperSpigot
					// -
					// Configurable
					// swimming
					// exhaustion
		...
		if (this.vehicle != null) {
			...
			if (i > 0) {
				if (this.vehicle instanceof EntityMinecartAbstract) {
					...
					} else if (this.e.c(MathHelper.floor(this.locX), MathHelper.floor(this.locY),
	...
	public int getExpToLevel() {
		return this.expLevel >= 30 ? 112 + (this.expLevel - 30) * 9
				: (this.expLevel >= 15 ? 37 + (this.expLevel - 15) * 5 : 7 + this.expLevel * 2);
	}
	...
	public IChatBaseComponent getScoreboardDisplayName() {
		...
		ChatComponentText chatcomponenttext = new ChatComponentText(
				ScoreboardTeam.getPlayerDisplayName(this.getScoreboardTeam(), this.getName()));

		chatcomponenttext.getChatModifier().setChatClickable(
				new ChatClickable(ChatClickable.EnumClickAction.SUGGEST_COMMAND, "/msg " + this.getName() + " "));
