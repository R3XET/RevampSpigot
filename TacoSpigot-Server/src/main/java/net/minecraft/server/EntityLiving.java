...
import eu.revamp.spigot.events.potion.PotionEffectExtendEvent;
...
public abstract class EntityLiving extends Entity {
    ...
    private boolean applyingSprintKnockback = false;

    private boolean applySprintKnockback;

    private Knockback knockbackProfile;

    public Knockback getKnockback() {
        return this.knockbackProfile;
    }

    public void setKnockback(Knockback profile) {
    ...
    //TODO ADDED CODE
    private int lastDamageByPlayerTick = 0;
    //TODO ADDED CODE
    ...
    public EntityLiving(World world) {
        ...
        /*
        if (this.isMob) {
            this.maxNoDamageTicks = RevampSpigot.getInstance().getMobMaxNoDamageTicks();
            if (this.isFromMobSpawner && RevampSpigot.getInstance().isMobStacking() && this instanceof EntityZombie && isBaby()) {
                EntityZombie entityZombie = (EntityZombie)this;
    ...
    protected void a(double d0, boolean flag, Block block, BlockPosition blockposition) {
        ...
        if (!this.world.isClientSide && this.fallDistance > 3.0F && flag) {
            ...
            if (block1.getMaterial() != Material.AIR) {
                ...
                //TODO ADDED CODE
                double d1 = FastMath.min(0.2F + f / 15.0F, 10.0F);
                //TODO ADDED CODE
    ...
    public void K() {
        ...
        //TODO ADDED CODE
        if (this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_ENTITY_STUCK_DAMAGE_CHECK == 0 && isAlive()){
            //TODO ADDED CODE
            if (this.inBlock()) {
                ...
                if (d0 < 0.0D) {
                    //TODO OLD CODE
                    //this.damageEntity(Dama
                    // geSource.STUCK, (float) Math.max(1, MathHelper.floor(-d0 * this.world.getWorldBorder().getDamageAmount())));
                    //TODO OLD CODE
                    //TODO ADDED CODE
                    damageEntity(DamageSource.STUCK, (float)FastMath.max(1.0D, FastMath.floor(-d0 * this.world.getWorldBorder().getDamageAmount())));
                    //TODO ADDED CODE
        ...
        //TODO ADDED CODE
        if (this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_ENTITY_WATER_CHECKS == 0 && isAlive()){
        //TODO ADDED CODE
            if (this.a(Material.WATER)) {
                if (!this.aY() && !this.hasEffect(MobEffectList.WATER_BREATHING.id) && !flag1) {
                    ...
                    if (this.getAirTicks() == -20) {
                        ...
                        //TODO ADDED CODE
                        if (Settings.IMP.SETTINGS.MISCELLANOUS.DO_ENTITIES_CREATE_WATER_SPLASH_PARTICLES)
                            for (int i = 0; i < 8; i++) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!mobeffect.tick(this)) {
                if (!this.world.isClientSide) {
                    ...
                    //TODO ADDED CODE
                    PotionEffectExpireEvent potionEffectExpireEvent = new PotionEffectExpireEvent((LivingEntity) getBukkitEntity(), new PotionUtils().getEffect(mobeffect));
                    this.world.getServer().getPluginManager().callEvent(potionEffectExpireEvent);
                    if (potionEffectExpireEvent.isCancelled()) {
                        new PotionUtils().extendDuration(mobeffect, potionEffectExpireEvent.getDuration());
                    } else {
                        iterator.remove();
                        this.b(mobeffect);
                    }
    ...
    public void addEffect(MobEffect mobeffect) {
        ...
        */
        ...
        if (this.d(mobeffect)) {
            if (this.effects.containsKey(mobeffect.getEffectId())) {
                PotionEffectExtendEvent event = new PotionEffectExtendEvent((LivingEntity)this.getBukkitEntity(), new PotionUtils().getEffect(mobeffect), new PotionUtils().getEffect((MobEffect)this.effects.get(mobeffect.getEffectId())));
                ...
                final PotionEffectAddEvent event2 = new PotionEffectAddEvent((LivingEntity)this.getBukkitEntity(), new PotionUtils().getEffect(mobeffect));
    ...
    public void removeEffect(int i) {
        ...
        */
        ...
        //TODO ADDED CODE
        if (mobeffect != null) {
            PotionEffectRemoveEvent event = new PotionEffectRemoveEvent((LivingEntity)this.getBukkitEntity(), new PotionUtils().getEffect(mobeffect));
            this.world.getServer().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
    ...
    public boolean damageEntity(DamageSource damagesource, float f) {
        ...
        if (damagesource.getEntity() instanceof EntityHuman)
            this.lastDamageByPlayerTick = MinecraftServer.currentTick;
        this.aB = 1.5F;
        boolean flag = true;
...
*/
    ...
    public void die(DamageSource damagesource) {
        if (!this.world.isClientSide) {
            int ticksSinceLastDamage = MinecraftServer.currentTick - this.lastDamageByPlayerTick;
            if (damagesource.equals(DamageSource.BURN) || damagesource.equals(DamageSource.FIRE) || damagesource.equals(DamageSource.LAVA) || (ticksSinceLastDamage <= RevampSpigot.getInstance().getTicksSinceLastPlayerDamageBeforeKillingStack() && !damagesource.equals(DamageSource.FALL))) {
                killOne(damagesource);
            } else {
                killStack();
            }
        }
...
/*
    public void killOne(DamageSource damageSource) {
        if (this.isFromMobSpawner && this.isMob && RevampSpigot.getInstance().isMobStacking() &&
                this instanceof EntityZombie && isBaby()) {
            EntityZombie entityZombie = (EntityZombie)this;
            entityZombie.setBaby(false);
        }
        Entity entity = damageSource.getEntity();
        ...
        this.world.broadcastEntityEffect(this, (byte)3);
        int newStackSize = --this.stackSize;
        if (newStackSize <= 0)
            return;
        newStack(newStackSize);
    ...
    public void killStack() {
        ...
        killThis();
        int newStackSize;
        while ((newStackSize = --this.stackSize) > 0)
            newStack(newStackSize).killThis();
        bs().g();
        this.world.broadcastEntityEffect(this, (byte)3);
        this.dead = true;
    }
*/
    public void killThis() {
        ...
        if (!this.world.isClientSide)
            if (ba() && this.world.getGameRules().getBoolean("doMobLoot")) {
                this.drops = new ArrayList<>();
                dropDeathLoot((this.lastDamageByPlayerTime > 0), 0);
                CraftEventFactory.callEntityDeathEvent(this, this.drops);
                this.drops = null;
            } else {
                CraftEventFactory.callEntityDeathEvent(this);
    ...
    public EntityLiving newStack(int newSize) {
        CraftEntity thisBukkitEntity = getBukkitEntity();
        CraftWorld craftWorld = (CraftWorld)thisBukkitEntity.getWorld();
        CraftEntity craftEntity = (CraftEntity)craftWorld.spawnEntity(thisBukkitEntity.getLocation(), thisBukkitEntity.getType());
        EntityLiving nmsEntity = (EntityLiving)craftEntity.getHandle();
        if (nmsEntity instanceof EntityZombie && nmsEntity.isBaby()) {
            EntityZombie entityZombie = (EntityZombie)nmsEntity;
            entityZombie.setBaby(false);
        }
        if (nmsEntity.vehicle != null)
            nmsEntity.vehicle.die();
        nmsEntity.vehicle = null;
        if (nmsEntity.passenger != null)
            nmsEntity.passenger.die();
        nmsEntity.passenger = null;
        nmsEntity.stackSize = newSize;
        nmsEntity.isMob = this.isMob;
        nmsEntity.fromMobSpawner = this.fromMobSpawner;
        nmsEntity.isFromMobSpawner = this.isFromMobSpawner;
        nmsEntity.portalCooldown = this.portalCooldown;
        nmsEntity.setInvulnerable(false);
        nmsEntity.justCreated = this.justCreated;
        nmsEntity.fireProof = this.fireProof;
        nmsEntity.fireTicks = this.fireTicks;
        nmsEntity.noDamageTicks = this.noDamageTicks;
        nmsEntity.lastDamageByPlayerTime = this.lastDamageByPlayerTime;
        nmsEntity.killer = this.killer;
        nmsEntity.combatTracker = this.combatTracker;
        //nmsEntity.updateName();
        return nmsEntity;
    }
    //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public void a(Entity entity, float f, double d0, double d1) {
        if (!Settings.IMP.SETTINGS.KNOCKBACK_PROFILES_ENABLED) {
            if (this.random.nextDouble() >= getAttributeInstance(GenericAttributes.c).getValue()) {
                ...
                this.motY /= 2.0D;
                this.motZ /= 2.0D;
                this.motX -= d0 / f2 * 0.4000000059604645D;
                this.motY += 0.4000000059604645D;
                this.motZ -= d1 / f2 * 0.4000000059604645D;
                if (this.motY > 0.4000000059604645D)
                    this.motY = 0.4000000059604645D;
            }
            return;
        ...
        if (this.random.nextDouble() >= getAttributeInstance(GenericAttributes.c).getValue()) {
            this.ai = true;
            //TODO OLD CODE
            //float f2 = (float)FastMath.sqrt(d0 * d0 + d1 * d1);
            //TODO OLD CODE
            ...
            if (this instanceof EntityHuman) {
                Knockback knockback = (this.knockback == null) ? RevampSpigot.getInstance().getKnockbackManager().getDefaultKnockback() : getKnockback();
                if (knockback.isFriction()) {
                    ...
                    double magnitude = FastMath.sqrt(d0 * d0 + d1 * d1);
                    //TODO ADDED CODE
                    this.motX /= knockback.getFrictions();
                    this.motY /= knockback.getFrictions();
                    this.motZ /= knockback.getFrictions();
                    this.motX -= d0 / magnitude * knockback.getHorizontal();
                    this.motY += knockback.getVertical();
                    this.motZ -= d1 / magnitude * knockback.getHorizontal();
    ...
    public boolean k_() {
        ...
        int j = (int)FastMath.floor((getBoundingBox()).b);
    ...
    public void e(float f, float f1) {
        ...
        if (i > 0) {
            ...
            //TODO ADDED CODE
            int j = (int)FastMath.floor(this.locX);
            int k = (int)FastMath.floor(this.locY - 0.20000000298023224D);
            int l = (int)FastMath.floor(this.locZ);
            //TODO ADDED CODE
       ...
       if (!this.isInvulnerable(damagesource)) {
            ...
            Function<Double, Double> absorption = new Function<Double, Double>() {
                ...
                public Double apply(Double f) {
                    ...
                    //TODO ADDED CODE
                    return -FastMath.max(f - FastMath.max(f - EntityLiving.this.getAbsorptionHearts(), 0.0D), 0.0D);
                    //TODO ADDED CODE
                }
            };
           ...
           //TODO ADDED CODE
           this.setAbsorptionHearts(FastMath.max(getAbsorptionHearts() - absorptionModifier, 0.0F));
           //TODO ADDED CODE
            if (f != 0.0F) {
                if (human) {
    ...
    protected void bF() {
        ...
        if (this.isSprinting()) {
            ...
            //TODO ADDED CODE
            this.motX -= FastMath.sin(f) * 0.20000000298023224D;
            this.motZ += FastMath.cos(f) * 0.20000000298023224D;
            //TODO ADDED CODE
        }
    ...
    public void g(float f, float f1) {
        ...
        if (this.bM()) {
            ...
            if (this.V() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
                ...
                if (this.onGround) {
                    ...
                    //TODO ADDED CODE
                    f5 = (this.world.getType(new BlockPosition((int)FastMath.floor(this.locX), (int)FastMath.floor((getBoundingBox()).b) - 1, (int)FastMath.floor(this.locZ))).getBlock()).frictionFactor * 0.91F;
                    //TODO ADDED CODE
                }
        ...
        //TODO ADDED CODE
        f2 = (float)FastMath.sqrt(d0 * d0 + d1 * d1) * 4.0F;
        //TODO ADDED CODE
        if (f2 > 1.0F) {
            f2 = 1.0F;
        }
    ...
    public void t_() {
        //TODO ADDED CODE
        //if (this.ticksLived % RevampSpigot.getInstance().getTicksBetweenSearch() == 0)
        //    searchNear();
        //TODO ADDED CODE
        super.t_();
        if (!this.world.isClientSide) {
