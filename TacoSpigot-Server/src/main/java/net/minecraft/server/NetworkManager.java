...
public class NetworkManager extends SimpleChannelInboundHandler<Packet> {
	...
	public static final Logger g = LogManager.getLogger();
	...
	public void channelActive(ChannelHandlerContext channelhandlercontext) throws Exception {
		...
		this.channel.config().setOption(ChannelOption.TCP_NODELAY,
				Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.TCP_NO_DELAY);
		this.channel.config().setOption(ChannelOption.ALLOCATOR, PooledByteBufAllocator.DEFAULT);
		this.channel.config().setOption(ChannelOption.WRITE_BUFFER_HIGH_WATER_MARK, 32 * 1024);
		this.channel.config().setOption(ChannelOption.WRITE_BUFFER_LOW_WATER_MARK, 8 * 1024);
		// Spigot Start
			...
			if (packet instanceof PacketPlayInCustomPayload) {
				...
				if ((name.equalsIgnoreCase("MC|BSign") || name.equalsIgnoreCase("MC|BEdit")) &&
					...
					if (data.length > 15000) {
						close(new ChatMessage("Invalid book packet"));
						return;
					}
					if (!this.openedBook) {
						close(new ChatMessage("Invalid book packet"));
			...
			if (this.m instanceof PlayerConnection)
				try {
					for (AsyncPacketHandler handler : RevampSpigot.getInstance().getPacketHandlers())
						handler.onPacketReceive((PlayerConnection)this.m, packet);
				} catch (Exception e) {
					e.printStackTrace();
				}
		}
	...
	//TODO ADDED CODE
/*
	private static final RevampSpigot SPIGOT = RevampSpigot.getInstance();
		...
		if (this.channel.isOpen()) {
			if (packet instanceof PacketPlayInCustomPayload) {
				...
				if ((name.equalsIgnoreCase("MC|BSign") || name.equalsIgnoreCase("MC|BEdit")) &&
					...
					byte[] data = payload.b().array();
					if (data.length > 15000) {
						sendStaffMessage(packet.getClass().getSimpleName(), 1L, "Too big payload data");
						SPIGOT.getLogger()
								.info("[RevampSpigot] {0} tried to crash server with payload crasher (-1)", getSocketAddress().toString());
						close(new ChatMessage("[RevampSpigot] Invalid payload data"));
						return;
					}
					if (!this.openedBook) {
						sendStaffMessage(packet.getClass().getSimpleName(), 1L,
								"Editing book without book opened");
						SPIGOT.getLogger()
								.info("[RevampSpigot] {0} tried to crash server with payload crasher (-2)", getSocketAddress().toString());
						close(new ChatMessage("[RevampSpigot] Invalid payload data"));
						return;
					}
					this.openedBook = false;
				...
				if (((PacketPlayInBlockPlace)packet).isInvalid()) {
					sendStaffMessage(packet.getClass().getSimpleName(), 1L,
							"BlockPlace book crasher");
					SPIGOT.getLogger()
							.info("[RevampSpigot] {0} tried to crash server with place book crasher (1)", getSocketAddress().toString());
					close(new ChatMessage("[RevampSpigot] Invalid blockplace data"));
					return;
				}
				ItemStack stack = ((PacketPlayInBlockPlace)packet).getItemStack();
				if (stack != null && stack.getItem() != null && stack.getItem().getName()
						.equalsIgnoreCase("item.writingBook"))
					this.openedBook = true;
			}
			try {
				packet.a(this.m);
			...
			if (this.m instanceof PlayerConnection)
				try {
					for (AsyncPacketHandler handler : SPIGOT.getPacketHandlers())
						handler.onPacketReceive((PlayerConnection)this.m, packet);
	...
	public void a(Packet packet, GenericFutureListener<? extends Future<? super Void>> genericfuturelistener,
		...
		if ((ThreadTypes.HIT_DETECTION.toBoolean()) && (g()) && ((packet instanceof PacketPlayInUseEntity))) {
			PacketPlayInUseEntity pI = (PacketPlayInUseEntity) packet;
			if (pI.a() == PacketPlayInUseEntity.EnumEntityUseAction.ATTACK) {
				RevampSpigot.getInstance().hdthread.addPacket(packet, this, null);
				return;
			}
		}
		if ((ThreadTypes.KNOCKBACK.toBoolean()) && (g()) && (((packet instanceof PacketPlayOutEntityVelocity || packet instanceof PacketPlayOutPosition))
				|| ((packet instanceof PacketPlayInPosition)) || ((packet instanceof PacketPlayInFlying)))) {
			RevampSpigot.getInstance().kbthread.addPacket(packet, this, null);
			return;
		}
		...
		if (this.g()) {
			...
			try {
				this.i.add(new NetworkManager.QueuedPacket(packet,
						ArrayUtils.add(agenericfuturelistener, 0, genericfuturelistener)));
			} finally {
				this.j.writeLock().unlock();
			}
		}
	...
	//TODO ADDED CODE
	private static final RevampThread ASYNC_THREADS = RevampSpigot.getInstance().getThread();

	public void a(Packet packet, GenericFutureListener<? extends Future<? super Void>> genericfuturelistener, GenericFutureListener... agenericfuturelistener) {
		if (Settings.IMP.SETTINGS.ASYNC.ASYNC_HIT_DETECTION && g() && packet instanceof PacketPlayInUseEntity) {
			PacketPlayInUseEntity pI = (PacketPlayInUseEntity)packet;
			if (pI.a() == PacketPlayInUseEntity.EnumEntityUseAction.ATTACK) {
				ASYNC_THREADS.getAsyncHitDetection().addPacket(packet, this, null);
				return;
			}
		}
		if (Settings.IMP.SETTINGS.ASYNC.ASYNC_KNOCKBACK_DETECTION && g() && (packet instanceof PacketPlayOutEntityVelocity ||
			...
			ASYNC_THREADS.getAsyncKnockbackDetection().addPacket(packet, this, null);
			return;
		}
		if (g()) {
			m();
			a(packet, (GenericFutureListener<? extends Future<? super Void>>[])ArrayUtils.add((Object[])agenericfuturelistener, 0, genericfuturelistener));
		} else {
			this.j.writeLock().lock();
			try {
				this.i.add(new QueuedPacket(packet, (GenericFutureListener<? extends Future<? super Void>>[])ArrayUtils.add((Object[])agenericfuturelistener, 0, genericfuturelistener)));
			} finally {
				this.j.writeLock().unlock();
			}
		}
	...
	/*
	 * We need to limit flush as it has a lot of syscalls which uses cpu and a lot
	 * of traffic
	 */
	...
	private void a(final Packet packet,
		...
		if (enumprotocol1 != enumprotocol) {
			...
			// this.channel.config().setAutoRead(false); // used for flow control but i dont
			// think weneed this tbh
		}
		...
		if (this.channel != null && this.channel.isOpen()) {
			...
			try {
				while (!this.i.isEmpty()) {
					NetworkManager.QueuedPacket networkmanager_queuedpacket = this.i
							.poll();
	...
	public void a(SecretKey secretkey) {
		...
		this.channel.pipeline().addBefore("splitter", "decrypt",
				new PacketDecrypter(MinecraftEncryption.a(2, secretkey)));
		this.channel.pipeline().addBefore("prepender", "encrypt",
				new PacketEncrypter(MinecraftEncryption.a(1, secretkey)));
	}
