...
public class BlockLever extends Block {
    ...
    //TODO ADDED CODE
    private static final Cache<UUID, Long> CACHED_USE = CacheBuilder.newBuilder().expireAfterWrite(10L, TimeUnit.MINUTES).build();
    //TODO ADDED CODE
    ...
    public boolean interact(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman, EnumDirection enumdirection, float f, float f1, float f2) {
        if (world.isClientSide) {
            ...
            //TODO ADDED CODE
            if (Settings.IMP.PACKET_LIMITER.LEVER_TIMESTAMP_ENABLED) {
                  Long time = CACHED_USE.getIfPresent(entityhuman.getUniqueID());
                 if (time != null && time > System.currentTimeMillis()) {
                         entityhuman.sendMessage(new ChatComponentText(Settings.IMP.PACKET_LIMITER.LEVER_MESSAGE));
                         return false;
                 }
                 CACHED_USE.put(entityhuman.getUniqueID(), System.currentTimeMillis() + Settings.IMP.PACKET_LIMITER.LEVER_TIMESTAMP);
                 }
            //TODO ADDED CODE
            ...
            world.makeSound((double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D, (double) blockposition.getZ() + 0.5D, "random.clock", 0.3F, iblockdata.get(BlockLever.POWERED).booleanValue() ? 0.6F : 0.5F);
