...
public class WorldData {
    ...
    //TODO ADDED CODE
    public final Map<BlockPosition, Integer> spongeLocationCache = new HashMap<>();
    //TODO ADDED CODE
    ...
    public WorldData(NBTTagCompound nbttagcompound) {
        ...
        if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.BETTER_SPONGE_MECHANICS && (
                !nbttagcompound.hasKey("spongeRadius") || nbttagcompound.getInt("spongeRadius") == Settings.IMP.SETTINGS.PERFORMANCE.TNT.SPONGE_RADIUS))
            if (nbttagcompound.hasKey("spongeCache")) {
                int[] data = nbttagcompound.getIntArray("spongeCache");
                if ((data.length & 0x3) == 0) {
                    ...
                    int arrayOfInt[], i;
                    byte b;
                    for (i = (arrayOfInt = data).length, b = 0; b < i; ) {
                        int dataPart = arrayOfInt[b];
                        int mod = index++ & 0x3;
                        if (mod == 0) {
                            x = dataPart;
                        } else if (mod == 1) {
                            ...
                            int integer = dataPart;
                            this.spongeLocationCache.put(new BlockPosition(x, y, z), integer);
                        }
                        b++;
                    }
                }
