...
public abstract class Entity implements ICommandListener {
    ...
    public boolean isTNTorSand;
    public int multiplier;
    public boolean skipVelocityMultiplier;
    public boolean processedInExplosion;
    ...
    public boolean isAddedToChunk() {
        ...
        //TODO ADDED CODE
        int chunkX = (int) FastMath.floor(this.locX / 16.0D);
        int chunkY = (int)FastMath.floor(this.locY / 16.0D);
        int chunkZ = (int)FastMath.floor(this.locZ / 16.0D);
        //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public boolean isFromMobSpawner = false;
...
/*
    public boolean isStack() {
        return (this.stackSize > 1 && RevampSpigot.getInstance().isMobStacking());
    }

    public boolean isStackable() {
        return (RevampSpigot.getInstance().isMobStacking() && this.isMob && this.isFromMobSpawner && isAlive() && this.stackSize < RevampSpigot.getInstance().getMaxStackSize() && !RevampSpigot.getInstance().getStackBlackListedEntityTypes().contains(getBukkitEntity().getType()));
    }
*/
/*
    public void searchNear() {
        ...
        double maximumDistance = RevampSpigot.getInstance().getStackRange(), maximumDistance1 = maximumDistance + 1.0D;
        List<Entity> entities = this.world.a(this, new AxisAlignedBB(MathHelper.floor(this.locX - maximumDistance), MathHelper.floor(this.locY - maximumDistance), MathHelper.floor(this.locZ - maximumDistance), MathHelper.floor(this.locX + maximumDistance1), MathHelper.floor(this.locY + maximumDistance1), MathHelper.floor(this.locZ + maximumDistance1)), entity -> (entity.isStackable() && entity.getBukkitEntity().getType() == type));
        for (Entity entity : entities) {
            if (!isStackable())
                break;
            int endSize = this.stackSize + entity.stackSize;
            if (endSize > RevampSpigot.getInstance().getMaxStackSize())
                continue;
            if (this.stackSize >= entity.stackSize) {
                this.stackSize = endSize;
                entity.die();
                updateName();
                if (this instanceof EntityZombie && ((EntityZombie)this).isBaby())
                    ((EntityZombie)this).setBaby(false);
                continue;
            }
            entity.stackSize = endSize;
            die();
            entity.updateName();
            if (entity instanceof EntityZombie && ((EntityZombie)entity).isBaby())
                ((EntityZombie)entity).setBaby(false);
            break;
        }
    }
    ...
    public void updateName() {
        ...
        setCustomName(RevampSpigot.getInstance().getMobNameTag().replace("{amount}", Integer.toString(this.stackSize)).replace("{mob_type}", getMobName()));
    }

    public String getMobName() {
        if (this instanceof EntityMagmaCube)
            return RevampSpigot.getInstance().getMagmaCubeName();
        if (this instanceof EntityCaveSpider)
            return RevampSpigot.getInstance().getCaveSpiderName();
        if (this instanceof EntityPigZombie)
            return RevampSpigot.getInstance().getZombiePigmanName();
        return getBukkitEntity().getType().getName();
    }*/
    //TODO ADDED CODE
    ...
    public Entity(World world) {
        ...
        this.isTNT = false;
        this.isSand = false;
        this.isTNTorSand = false;
        this.processedInExplosion = false;
    ...
    public Entity(World world, boolean isTNT, boolean isSand) {
        this.canExplode = true;
        this.isTNT = false;
        this.isSand = false;
        this.isTNTorSand = false;
        this.inUnloadedChunk = false;
        this.loadChunks = false;
        this.activationType = ActivationRange.initializeEntityActivationType(this);
        this.activatedTick = Integer.MIN_VALUE;
        this.processedInExplosion = false;
        this.numCollisions = 0;
        if (isTNT) {
            this.isTNT = true;
            this.isTNTorSand = true;
        }
        if (isSand) {
            this.isSand = true;
            this.isTNTorSand = true;
    ...
    public void checkDisabled() {
        ...
        if (craftEntity != null && craftEntity.getType() != null && RevampSpigot.getInstance().getDisabledEntityTypes().contains(craftEntity.getType()))
            die();
    }*/
    //TODO ADDED CODE
    ...
    public void K() {
        //TODO ADDED CODE
        /*if (this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_DISABLED_ENTITY_CHECK == 0)
            checkDisabled();*/
        //TODO ADDED CODE
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.noclip) {
            ...
            final boolean axisScan = Settings.IMP.SETTINGS.PERFORMANCE.TNT.OPTIMIZE_TNT_MOVEMENT && xLength * yLength * zLength > 10.0;
            ...
            boolean flag2 = this.onGround || (d7 != d1 && d7 < 0.0);
            if (Settings.IMP.SETTINGS.PERFORMANCE.TNT.FIX_EAST_WEST_CANOONS && Math.abs(d0) > Math.abs(d2)) {
                if (axisScan) {
            ...
            int j = (int)FastMath.floor(this.locY - 0.20000000298023224D);
            ...
            if (positionChanged && getBukkitEntity() instanceof Vehicle) {
                ...
                //TODO ADDED CODE
                org.bukkit.block.Block bl = this.world.getWorld().getBlockAt((int)FastMath.floor(this.locX), (int)FastMath.floor(this.locY), (int)FastMath.floor(this.locZ));
                //TODO ADDED CODE
    ...
    //TODO ADDED CODE
    public double scanX(double motX, boolean insideBorder, boolean special) {
        if (motX == 0.0D)
            return 0.0D;
        AxisAlignedBB bb = this.boundingBox.a(motX, 0.0D, 0.0D);
        int minX = MathHelper.floor(bb.a), maxX = MathHelper.floor(bb.d) + 1;
        int minY = MathHelper.floor(bb.b), maxY = MathHelper.floor(bb.e);
        int minZ = MathHelper.floor(bb.c), maxZ = MathHelper.floor(bb.f);
        List<AxisAlignedBB> list = new ArrayList<>(8);
        if (motX > 0.0D) {
            int cxstart = minX >> 4, cxend = maxX - 1 >> 4;
            int czstart = minZ >> 4, czend = maxZ >> 4;
            for (int chunkx = cxstart; chunkx <= cxend; chunkx++) {
                SmallCachedRegion region = new SmallCachedRegion(this.world, chunkx, chunkx, czstart, czend, true);
                int cx = chunkx << 4;
                int xstart = (minX < cx) ? cx : minX;
                int xend = (maxX < cx + 16) ? maxX : (cx + 16);
                for (int x = xstart; x < xend; x++) {
                    if (scanXSection(maxZ, minZ, maxY, minY, insideBorder, special, region, x, list, bb)) {
                        motX = getMotX(motX, list, true);
                        if (!list.isEmpty())
                            return motX;
                    }
                }
            ...
            int cxstart = maxX - 1 >> 4, cxend = minX >> 4;
            int czstart = maxZ >> 4, czend = minZ >> 4;
            for (int chunkx = cxstart; chunkx >= cxend; chunkx--) {
                SmallCachedRegion region = new SmallCachedRegion(this.world, chunkx, chunkx, czend, czstart, true);
                int cx = chunkx << 4;
                int xstart = ((maxX < cx + 16) ? maxX : (cx + 16)) - 1;
                int xend = (minX < cx) ? cx : minX;
                for (int x = xstart; x >= xend; x--) {
                    if (scanXSection(maxZ, minZ, maxY, minY, insideBorder, special, region, x, list, bb)) {
                        motX = getMotX(motX, list, true);
                        if (!list.isEmpty())
    ...
    public double scanY(double motY, boolean insideBorder, boolean special) {
        if (motY == 0.0D)
            return 0.0D;
        AxisAlignedBB bb = this.boundingBox.a(0.0D, motY, 0.0D);
        int minX = MathHelper.floor(bb.a), maxX = MathHelper.floor(bb.d);
        int minZ = MathHelper.floor(bb.c), maxZ = MathHelper.floor(bb.f);
        List<AxisAlignedBB> list = new ArrayList<>(8);
        SmallCachedRegion region = new SmallCachedRegion(this.world, minX >> 4, maxX >> 4, minZ >> 4, maxZ >> 4, true);
        if (motY > 0.0D) {
            int ystart = MathHelper.floor(bb.b) - 1;
            int yend = MathHelper.floor(bb.e) + 1;
            if (yend > this.world.getHeight())
                yend = this.world.getHeight() + 1;
            for (int y = ystart; y < yend; y++) {
                if (scanYSection(maxX, minX, maxZ, minZ, insideBorder, special, region, y, list, bb)) {
                    motY = getMotY(motY, list, true);
                    if (!list.isEmpty())
                        return motY;
                }
            }
        } else {
            int ystart = MathHelper.floor(bb.e);
            int yend = MathHelper.floor(bb.b) - 1;
            if (yend < 0)
                yend = 0;
            for (int y = ystart; y >= yend; y--) {
                if (scanYSection(maxX, minX, maxZ, minZ, insideBorder, special, region, y, list, bb)) {
    ...
    public double scanZ(double motZ, boolean insideBorder, boolean special) {
        if (motZ == 0.0D)
            return 0.0D;
        AxisAlignedBB bb = this.boundingBox.a(0.0D, 0.0D, motZ);
        int minX = MathHelper.floor(bb.a), maxX = MathHelper.floor(bb.d);
        int minY = MathHelper.floor(bb.b), maxY = MathHelper.floor(bb.e);
        int minZ = MathHelper.floor(bb.c), maxZ = MathHelper.floor(bb.f) + 1;
        List<AxisAlignedBB> list = new ArrayList<>(8);
        if (motZ > 0.0D) {
            int cxstart = minX >> 4, cxend = maxX >> 4;
            int czstart = minZ >> 4, czend = maxZ - 1 >> 4;
            for (int chunkz = czstart; chunkz <= czend; chunkz++) {
                SmallCachedRegion region = new SmallCachedRegion(this.world, cxstart, cxend, chunkz, chunkz, true);
                int cz = chunkz << 4;
                int zstart = (minZ < cz) ? cz : minZ;
                int zend = (maxZ < cz + 16) ? maxZ : (cz + 16);
                for (int z = zstart; z < zend; z++) {
                    if (scanZSection(maxX, minX, maxY, minY, insideBorder, special, region, z, list, bb)) {
                        motZ = getMotZ(motZ, list, true);
                        if (!list.isEmpty())
                            return motZ;
                    }
                }
            ...
            int cxstart = maxX >> 4, cxend = minX >> 4;
            int czstart = maxZ - 1 >> 4, czend = minZ >> 4;
            for (int chunkz = czstart; chunkz >= czend; chunkz--) {
                SmallCachedRegion region = new SmallCachedRegion(this.world, cxend, cxstart, chunkz, chunkz, true);
                int cz = chunkz << 4;
                int zstart = ((maxZ < cz + 16) ? maxZ : (cz + 16)) - 1;
                int zend = (minZ < cz) ? cz : minZ;
                for (int z = zstart; z >= zend; z--) {
                    if (scanZSection(maxX, minX, maxY, minY, insideBorder, special, region, z, list, bb)) {
                        motZ = getMotZ(motZ, list, true);
                        if (!list.isEmpty())
    ...
    private boolean scanXSection(int maxZ, int minZ, int maxY, int minY, boolean insideBorder, boolean special, SmallCachedRegion region, int x, List<AxisAlignedBB> list, AxisAlignedBB bb) {
        for (int z = maxZ; z >= minZ; z--) {
            for (int y = maxY; y >= minY; y--)
                this.world.checkLocation(this.g, insideBorder, special, this, region, x, y, z, list, bb);
        }
        return !list.isEmpty();
    }

    private boolean scanYSection(int maxX, int minX, int maxZ, int minZ, boolean insideBorder, boolean special, SmallCachedRegion region, int y, List<AxisAlignedBB> list, AxisAlignedBB bb) {
    ...
    private boolean scanZSection(int maxX, int minX, int maxY, int minY, boolean insideBorder, boolean special, SmallCachedRegion region, int z, List<AxisAlignedBB> list, AxisAlignedBB bb) {
        for (int x = maxX; x >= minX; x--) {
            for (int y = maxY; y >= minY; y--)
                this.world.checkLocation(this.g, insideBorder, special, this, region, x, y, z, list, bb);
        }
    ...
    public double getMotX(double motX, List<AxisAlignedBB> blocks, boolean clear) {
        double motXOld = motX;
        for (AxisAlignedBB block : blocks)
            motX = block.a(this.boundingBox, motX);
        if (clear && motXOld == motX)
            blocks.clear();
        return motX;
    }

    public double getMotY(double motY, List<AxisAlignedBB> blocks, boolean clear) {
        double motYOld = motY;
        for (AxisAlignedBB block : blocks)
            motY = block.b(this.boundingBox, motY);
        if (clear && motYOld == motY)
            blocks.clear();
        return motY;
    }

    public double getMotZ(double motZ, List<AxisAlignedBB> blocks, boolean clear) {
        double motZOld = motZ;
        for (AxisAlignedBB block : blocks)
            motZ = block.c(this.boundingBox, motZ);
        if (clear && motZOld == motZ)
            blocks.clear();
        return motZ;
    }
    //TODO ADDED CODE
    ...
    protected void X() {
        ...
        //TODO ADDED CODE
        float f = (float)FastMath.sqrt(this.motX * this.motX * 0.20000000298023224D + this.motY * this.motY + this.motZ * this.motZ * 0.20000000298023224D) * 0.2F;
        //TODO ADDED CODE
        ...
        //TODO ADDED CODE
        float f1 = (float)FastMath.floor((getBoundingBox()).b);
        //TODO ADDED CODE
        ...
        for (i = 0; (float) i < 1.0F + this.width * 20.0F; ++i) {
            ...
            this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX + (double) f2, f1 + 1.0F, this.locZ + (double) f3, this.motX, this.motY - (double) (this.random.nextFloat() * 0.2F), this.motZ);
        }

        for (i = 0; (float) i < 1.0F + this.width * 20.0F; ++i) {
            f2 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
            f3 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
            this.world.addParticle(EnumParticle.WATER_SPLASH, this.locX + (double) f2, f1 + 1.0F, this.locZ + (double) f3, this.motX, this.motY, this.motZ);
        }
    ...
    public void a(float f, float f1, float f2) {
        ...
        if (f3 >= 1.0E-4F) {
            ...
            //TODO ADDED CODE
            float f4 = (float)FastMath.sin((this.yaw * 3.1415927F / 180.0F));
            float f5 = (float)FastMath.cos((this.yaw * 3.1415927F / 180.0F));
            //TODO ADDED CODE
    ...
    protected final Vec3D f(float f, float f1) {
        ...
        */
        ...
        //TODO ADDED CODE
        float f2 = (float)FastMath.cos((-f1 * 0.017453292F - 3.1415927F));
        float f3 = (float)FastMath.sin((-f1 * 0.017453292F - 3.1415927F));
        float f4 = -((float)FastMath.cos((-f * 0.017453292F)));
        float f5 = (float)FastMath.sin((-f * 0.017453292F));
        //TODO ADDED CODE
    ...
    public boolean inBlock() {
        if (this.noclip) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                //TODO ADDED CODE
                int j = (int)FastMath.floor(this.locY + ((((i >> 0) % 2) - 0.5F) * 0.1F) + getHeadHeight());
                int k = (int)FastMath.floor(this.locX + ((((i >> 1) % 2) - 0.5F) * this.width * 0.8F));
                int l = (int)FastMath.floor(this.locZ + ((((i >> 2) % 2) - 0.5F) * this.width * 0.8F));
    ...
    public void d(BlockPosition blockposition) {
        if (this.portalCooldown > 0) {
            ...
            if (!this.world.isClientSide && !blockposition.equals(this.an)) {
                ...
                //TODO ADDED CODE
                d1 = FastMath.abs(MathHelper.c(d1 - ((shapedetector_shapedetectorcollection.b().e().c() == EnumDirection.EnumAxisDirection.NEGATIVE) ? 1 : 0), d0, d0 - shapedetector_shapedetectorcollection.d()));
                //TODO ADDED CODE
    ...
    public EnumDirection getDirection() {
        ...
        //TODO ADDED CODE
        return EnumDirection.fromType2((int)FastMath.floor((this.yaw * 4.0F / 360.0F) + 0.5D) & 0x3);
        //TODO ADDED CODE
    }
