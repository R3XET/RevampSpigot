...
public class EntityArrow extends Entity implements IProjectile {
    ...
    public EntityArrow(World world, EntityLiving entityliving, EntityLiving entityliving1, float f, float f1) {
        ...
        //TODO ADDED CODE
        double d3 = FastMath.sqrt(d0 * d0 + d2 * d2);
        //TODO ADDED CODE
    ...
    public EntityArrow(World world, EntityLiving entityliving, float f) {
        ...
        */
        ...
        //TODO ADDED CODE
        this.locX -= FastMath.cos((this.yaw / 180.0F * 3.1415927F)) * 0.1599999964237213D;
        this.locY -= 0.10000000149011612D;
        this.locZ -= FastMath.sin((this.yaw / 180.0F * 3.1415927F)) * 0.1599999964237213D;
        setPosition(this.locX, this.locY, this.locZ);
        this.motX = -FastMath.sin((this.yaw / 180.0F * 3.1415927F)) * MathHelper.cos(this.pitch / 180.0F * 3.1415927F);
        this.motZ = FastMath.cos((this.yaw / 180.0F * 3.1415927F)) * MathHelper.cos(this.pitch / 180.0F * 3.1415927F);
        this.motY = -FastMath.sin((this.pitch / 180.0F * 3.1415927F));
        //TODO ADDED CODE
    ...
    public void shoot(double d0, double d1, double d2, float f, float f1) {
        ...
        //TODO ADDED CODE
        float f2 = (float)FastMath.sqrt(d0 * d0 + d1 * d1 + d2 * d2);
        //TODO ADDED CODE
        ...
        //TODO ADDED CODE
        float f3 = (float)FastMath.sqrt(d0 * d0 + d2 * d2);
        //TODO ADDED CODE
    ...
    public void t_() {
        ...
        if (this.lastPitch == 0.0F && this.lastYaw == 0.0F) {
            ...
            //TODO ADDED CODE
            float f = (float)FastMath.sqrt(this.motX * this.motX + this.motZ * this.motZ);
            //TODO ADDED CODE
                        ...
                        */
                        ...
                        if (movingobjectposition.entity instanceof EntityLiving) {
                            ...
                            if (movingobjectposition.entity.getBukkitEntity().getType() == EntityType.PLAYER) {
                                EntityPlayer hit = (EntityPlayer)movingobjectposition.entity; //TODO FIXARE EXCEPTION
                                if (this.shooter != null && this.shooter != hit && this.shooter instanceof EntityPlayer)
                                    ((EntityPlayer)this.shooter).playerConnection.sendPacket(new PacketPlayOutGameStateChange(6, 0.0F));
                                Knockback knockback = (hit.getKnockback() == null) ? RevampSpigot.getInstance().getKnockbackManager().getDefaultKnockback() : hit.getKnockback();
                                Vector v = (new Vector(this.motX, this.motY, this.motZ)).normalize();
                                double velX = v.getX() / 1.6D * knockback.getBowHorizontal();
                                double velY = 0.36D * knockback.getBowVertical();
                                double velZ = v.getZ() / 1.6D * knockback.getBowHorizontal();
                                if (this.knockbackStrength > 0) {
                                    velX *= (this.knockbackStrength + 1);
                                    velY *= (this.knockbackStrength + 1);
                                    velZ *= (this.knockbackStrength + 1);
                                }
                                ...
                                if (this.knockbackStrength > 0) {
                                    float f4 = (float) FastMath.sqrt(this.motX * this.motX + this.motZ * this.motZ);
                                    if (f4 > 0.0F)
                                        movingobjectposition.entity.g(this.motX * this.knockbackStrength * 0.6000000238418579D / f4, 0.1D, this.motZ * this.knockbackStrength * 0.6000000238418579D / f4);
                                }
                                if (this.shooter instanceof EntityLiving) {
                                    EnchantmentManager.a(entityliving, this.shooter);
                                    EnchantmentManager.b((EntityLiving)this.shooter, entityliving);
