...
public class EntityItem extends Entity implements HopperPusher {
    ...
    public void t_() {
        ...
        if (itemStack == null) {
            ...
            super.t_();
            if (this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_HOPPER_SEARCH == 0 &&
                    tryPutInHopper())
                return;
            ...
            if (((!Settings.IMP.SETTINGS.MISCELLANOUS.DISABLE_ITEM_BURN_CHECK_ON_MOVE && flag) || this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_ITEM_BURN_CHECK == 0) &&
                    this.world.getType(new BlockPosition(this)).getBlock().getMaterial() == Material.LAVA) {
                this.motY = 0.20000000298023224D;
            ...
            if (((!Settings.IMP.SETTINGS.MISCELLANOUS.DISABLE_MERGE_CHECK_ON_ITEM_MOVE && flag) || this.ticksLived % Settings.IMP.SETTINGS.PERFORMANCE.TICKS.TICKS_BETWEEN_ITEM_MERGE_SEARCH == 0) &&
            ...
            //TODO ADDED CODE
            f = (this.world.getType(new BlockPosition(FastMath.floor(this.locX), FastMath.floor((getBoundingBox()).b) - 1.0D, FastMath.floor(this.locZ))).getBlock()).frictionFactor * 0.98F;
            //TODO ADDED CODE
            ...
            if (this.onGround)
                this.motY *= -0.5D;
            if (!Settings.IMP.SETTINGS.MISCELLANOUS.DISABLE_ITEM_WATER_PARTICLES_CHECK)
                W();
    ...
    */
    ...
    //TODO ADDED CODE
    private void w(ItemStack itemStack) {
        if (itemStack.count >= itemStack.getMaxStackSize() && Settings.IMP.SETTINGS.MISCELLANOUS.DISABLE_MERGE_SEARCH_FOR_MAX_STACKED_ITEMS)
            return;
        double radius = this.world.spigotConfig.itemMerge;
        for (EntityItem entityitem : this.world.a(EntityItem.class, getBoundingBox().grow(radius, radius, radius))) {
            a(entityitem);
        }
    }
    //TODO ADDED CODE
