...
public class PlayerInteractManager {
    ...
    public boolean breakBlock(BlockPosition blockposition) {
        ...
        if (this.player instanceof EntityPlayer) {
            ...
            //TODO FINIRE
            /*
            if (!event.isCancelled() && RevampSpigotConfig.betterSponges) {
                org.bukkit.Material mat = block.getType();
                if (mat == org.bukkit.Material.SPONGE || (Settings.IMP.SETTINGS.ENTITY.SPAWNERS_ACTS_AS_SPONGES && mat == org.bukkit.Material.MOB_SPAWNER)) {
                    for (CoordinateInt value : TNTOptimization.spongeValues) {
                        org.bukkit.block.Block b = block.getRelative(value.x, value.y, value.z);
                        org.bukkit.Material type = b.getType();
                        if (type == org.bukkit.Material.STATIONARY_WATER || type == org.bukkit.Material.WATER || type == org.bukkit.Material.STATIONARY_LAVA || type == org.bukkit.Material.LAVA) {
                            int id = type.getId();
                            byte data = b.getData();
