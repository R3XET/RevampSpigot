...
public class PlayerConnection implements PacketListenerPlayIn, IUpdatePlayerListBox {
    ...
    private boolean checkPacketLimit(Packet packet) {
        if (Settings.IMP.PACKET_LIMITER.ALLOW_SCHEMATIC_PRINTER)
            return false;
        long start = System.currentTimeMillis();
        if (Settings.IMP.PACKET_LIMITER.ENABLED) {
            if (this.packetsPerSecondTime < System.currentTimeMillis()) {
                this.packetsPerSecondTime = System.currentTimeMillis() + 1000L;
                this.packetsPerSecond = 0;
            }
            if (++this.packetsPerSecond > Settings.IMP.PACKET_LIMITER.PACKETS_PER_SECOND) {
                disconnect(CC.AQUA + "RevampSpigot -> Too many packets!");
                RevampSpigot.getInstance().getLogger().info("{0} -> too many packets per second! ({1})", this.player.getName(), this.packetsPerSecond);
                sendStaffMessage(packet.getClass().getSimpleName(), System.currentTimeMillis() - start, "Too many packets (" + this.packetsPerSecond + " > " + Settings.IMP.PACKET_LIMITER.PACKETS_PER_SECOND + ")");
                return true;
            }
        }
        return false;
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (this.b(packetplayinflying)) {
            ...
            if (!this.player.viewingCredits) {
                ...
                //TODO ADDED CODE
                double delta = FastMath.pow(this.lastPosX - to.getX(), 2.0D) + FastMath.pow(this.lastPosY - to.getY(), 2.0D) + FastMath.pow(this.lastPosZ - to.getZ(), 2.0D);
                ...
                if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
                    ...
                    if (Settings.IMP.SETTINGS.PERFORMANCE.FIRE_MOVE_EVENTS) { // Spigot - don't skip any move events
    ...
    public void a(PacketPlayInBlockPlace packetplayinblockplace) {
        ...
        if (this.disconnecting)
            return;
        Settings settings = Settings.IMP;
        if (checkPacketLimit(packetplayinblockplace))
            return;
        long start = System.currentTimeMillis();
        if (packetplayinblockplace.isInvalid()) {
            RevampSpigot.getInstance().getLogger().info("{0} Invalid NBT item (" + packetplayinblockplace.getClass().getSimpleName() + ")!", this.player.getName());
            disconnect("[RevampSpigot] -> Invalid packet received!");
            sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid NBT item");
            return;
        }
        ...
        ItemStack placeItem = packetplayinblockplace.getItemStack();
        if (placeItem != null) {
            if (settings.SETTINGS.DEVELOPER_SETTINGS.DEBUG)
                RevampSpigot.getInstance().getLogger().info("{0} Packet: {1} [Item: {2} | tag: {3}]", this.player.getName(), packetplayinblockplace.getClass().getSimpleName(), placeItem.getItem().getName(), placeItem.getTag().toString());
            if (placeItem.getItem() instanceof ItemWrittenBook || placeItem.getItem() instanceof ItemBookAndQuill) {
                if (itemstack == null || (!(itemstack.getItem() instanceof ItemWrittenBook) && !(itemstack.getItem() instanceof ItemBookAndQuill))) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to place a book but doesn't have any book in inventory!", this.player.getName());
                    disconnect("[RevampSpigot] -> Invalid packet sent!");
                    sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Packet item do not match with hand item");
                    return;
                }
                if (placeItem.getItem() instanceof ItemWrittenBook) {
                    if (!ItemWrittenBook.isValid(placeItem.getTag())) {
                        RevampSpigot.getInstance().getLogger().info("{0} Tried to place invalid item!", this.player.getName());
                        disconnect("[RevampSpigot] -> Invalid packet sent!");
                        sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid Book data v1");
                        return;
                    }
                } else if (!ItemBookAndQuill.isValid(placeItem.getTag())) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to place invalid item!", this.player.getName());
                    disconnect("[RevampSpigot] -> Invalid packet sent!");
                    sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book data v2");
                    return;
                }
                int length = placeItem.getTag().toString().length();
                int maxLength = settings.SETTINGS.ANTICRASH.BOOK_NBT_LIMIT;
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in blockPlace! Item: book | Length: {1}, max allowed: {2}", this.player.getName(), length, maxLength);
                    disconnect("[RevampSpigot] -> Invalid item sent!");
                    sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book nbt size");
                    return;
                }
            } else if (placeItem.getItem() instanceof ItemFireworks || placeItem.getItem() instanceof ItemFireworksCharge) {
                int length = placeItem.getTag().toString().length();
                int maxLength = settings.SETTINGS.ANTICRASH.FIREWORK_NBT_LIMIT;
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in blockPlace! Item: firework | Length: {1}, max allowed: {2}", this.player.getName(), length, maxLength);
                    disconnect("[RevampSpigot] -> Invalid item sent!");
                    sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid firework item (too big)");
                    return;
                }
            }
        }*/
        ...
        //TODO ADDED CODE
        if (this.disconnecting) return;
        if (checkPacketLimit(packetplayinblockplace)) return;
        long start = System.currentTimeMillis();
        if (packetplayinblockplace.isInvalid()) {
            RevampSpigot.getInstance().getLogger().info("{0} Invalid NBT item (" + packetplayinblockplace.getClass().getSimpleName() + ")!", this.player.getName());
            disconnect(CC.AQUA + "RevampSpigot -> Invalid packet received!");
            sendStaffMessage(packetplayinblockplace.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid NBT item");
            return;
        }*/
        //TODO ADDED CODE
    ...
    // Spigot end
        ...
        if (packetplayinblockplace.getFace() == 255) {
            ...
            */
            //TODO OLD CODE
                //TODO ADDED CODE
                float f3 = (float)FastMath.cos((-f2 * 0.017453292F - 3.1415927F));
                float f4 = (float)FastMath.sin((-f2 * 0.017453292F - 3.1415927F));
                float f5 = (float)-FastMath.cos((-f1 * 0.017453292F));
                float f6 = (float)FastMath.sin((-f1 * 0.017453292F));
                //TODO ADDED CODE
    ...
    */
    ...
    public void sendPacket(Packet packet) {
        ...
        try {
            this.networkManager.handle(packet);
            for (AsyncPacketHandler handler : RevampSpigot.getInstance().getPacketHandlers()) {
                try {
                    handler.onPacketSent(this, packet);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
...
/*
        //TODO ADDED CODE
        if (this.disconnecting) return;
        if (checkPacketLimit(packetplayinarmanimation)) return;
        if (Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.DEBUG) {
            RevampSpigot.getInstance().getLogger().info("{0} Packet: {1}", this.player.getName(), packetplayinarmanimation.getClass().getSimpleName());
        }
        if (Settings.IMP.PACKET_LIMITER.ARM_ANIM_ENABLED) {
            if (this.lastSwingTick != MinecraftServer.currentTick) {
                this.swings = 0;
                this.lastSwingTick = MinecraftServer.currentTick;
            } else {
                if (this.swings > 5) {
                    if (Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.DEBUG) {
                        RevampSpigot.getInstance().getLogger().info("[DEBUG] {0} Packet: {1} swing cancelled", this.player.getName(), packetplayinarmanimation.getClass().getSimpleName());
                    }
                    if (MinecraftServer.currentTick - 1 == this.lastSequentialSwingTick) {
                        this.sequentialSwingRateLimits++;
                    } else if (MinecraftServer.currentTick != this.lastSequentialSwingTick) {
                        this.sequentialSwingRateLimits = 0;
                    } else if (75 < this.sequentialSwingRateLimits) {
                        disconnect(CC.AQUA + "RevampSpigot -> Invalid arm animations");
                        sendStaffMessage(packetplayinarmanimation.getClass().getSimpleName(), 1L, "Too fast arm animations");
                    }
                    this.lastSequentialSwingTick = MinecraftServer.currentTick;
                    return;
                }
                this.swings++;
            }
            if (this.canArmAnimAt > System.currentTimeMillis()) {
                if (Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.DEBUG) {
                    RevampSpigot.getInstance().getLogger().info("[DEBUG] {0} Packet: {1} cancelled", this.player.getName(), packetplayinarmanimation.getClass().getSimpleName());
                }
                return;
            }
            this.canArmAnimAt = System.currentTimeMillis() + Settings.IMP.PACKET_LIMITER.ARM_ANIMATION_TIMESTAMP;
        }
        //TODO ADDED CODE
...
/*
        //TODO ADDED CODE
        if (this.disconnecting) return;
        if (checkPacketLimit(packetplayinuseentity)) return;
        //TODO ADDED CODE
*/
    ...
    public void a(PacketPlayInWindowClick packetplayinwindowclick) {/*
        ...
        if (this.disconnecting)
            return;
        if (checkPacketLimit(packetplayinwindowclick))
            return;
        long start = System.currentTimeMillis();
        if (packetplayinwindowclick.isInvalid()) {
            RevampSpigot.getInstance().getLogger().info("{0} Invalid NBT item (" + packetplayinwindowclick.getClass().getSimpleName() + ")!", this.player.getName());
            disconnect("[RevampSpigot] -> Invalid packet received!");
            sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid NBT item");
            return;
        }
        ItemStack windowItem = packetplayinwindowclick.e();
        if (windowItem != null) {
            if (Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.DEBUG)
                RevampSpigot.getInstance().getLogger().info("{0} Packet: {1} [Item: {2} | tag: {3}]", this.player.getName(), packetplayinwindowclick.getClass().getSimpleName(), windowItem.getItem().getName(), windowItem.getTag().toString());
            if (windowItem.getItem() instanceof ItemWrittenBook || windowItem.getItem() instanceof ItemBookAndQuill) {
                if (windowItem.getItem() instanceof ItemWrittenBook) {
                    if (!ItemWrittenBook.isValid(windowItem.getTag())) {
                        RevampSpigot.getInstance().getLogger().info("{0} Tried to click in invalid item!", this.player.getName());
                        disconnect("[RevampSpigot] -> Invalid packet sent!");
                        sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book item v1");
                        return;
                    }
                } else if (!ItemBookAndQuill.isValid(windowItem.getTag())) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to click in invalid item!", this.player.getName());
                    disconnect("[RevampSpigot] -> Invalid packet sent!");
                    sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book item v2");
                    return;
                }
                int length = windowItem.getTag().toString().length();
                int maxLength = Settings.IMP.SETTINGS.ANTICRASH.BOOK_NBT_LIMIT;
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in windowClick! Item: book | Length: {1}, max allowed: {2}", this.player.getName(), Integer.valueOf(length), Integer.valueOf(maxLength));
                    disconnect("[RevampSpigot] -> Invalid item sent!");
                    sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book nbt size");
                    return;
                }
            } else if (windowItem.getItem() instanceof ItemFireworks || windowItem.getItem() instanceof ItemFireworksCharge) {
                int length = windowItem.getTag().toString().length();
                int maxLength = Settings.IMP.SETTINGS.ANTICRASH.FIREWORK_NBT_LIMIT;
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in windowClick! Item: firework | Length: {1}, max allowed: {2}", this.player.getName(), Integer.valueOf(length), Integer.valueOf(maxLength));
                    disconnect("[RevampSpigot] -> Invalid item sent!");
                    sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid firework data");
                    return;
                }
            }
        }*/
        ...
        if (windowItem != null) {
            ...
            if (windowItem.getItem() instanceof ItemWrittenBook || windowItem.getItem() instanceof ItemBookAndQuill) {
                if (windowItem.getItem() instanceof ItemWrittenBook) {
                    if (!ItemWrittenBook.isValid(windowItem.getTag())) {
                        RevampSpigot.getInstance().getLogger().info("{0} Tried to clock in invalid item!", this.player.getName());
                        disconnect(CC.AQUA + "RevampSpigot -> Invalid packet sent!");
                        sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book item v1");
                        return;
                    }
                } else if (!ItemBookAndQuill.isValid(windowItem.getTag())) {
                ...
                int length = windowItem.getTag().toString().length();
                int maxLength = RevampSpigot.getInstance().getAntiCrashBookNbtLimit();
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in windowClick! Item: book | Length: {1}, max allowed: {2}", this.player.getName(), length, maxLength);
                    disconnect(CC.AQUA + "RevampSpigot -> Invalid item sent!");
                    sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book nbt size");
                    return;
                }
            } else if (windowItem.getItem() instanceof ItemFireworks || windowItem.getItem() instanceof ItemFireworksCharge) {
                int length = windowItem.getTag().toString().length();
                int maxLength = RevampSpigot.getInstance().getAntiCrashFireworkNbtLimit();
                if (length > maxLength) {
                    RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in windowClick! Item: firework | Length: {1}, max allowed: {2}", this.player.getName(), length, maxLength);
                    disconnect(CC.AQUA + "RevampSpigot -> Invalid item sent!");
                    sendStaffMessage(packetplayinwindowclick.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid firework data");
                    return;
                }
        ...
        if (this.player.activeContainer.windowId == packetplayinwindowclick.a() && this.player.activeContainer.c(this.player)) {
            ...
            if (false) { // this.player.isSpectator()) {
                ...
                if (packetplayinwindowclick.f() != 5) {
                    ...
                    //TODO ADDED CODE
                    if (event.getWhoClicked() instanceof Player && inventory != null && (inventory.getType() == InventoryType.CRAFTING || inventory.getType() == InventoryType.PLAYER) && (event.getSlotType() == InventoryType.SlotType.ARMOR || event.isShiftClick())) {
                        ArmorEvents.check((Player)event.getWhoClicked());
                    }
                    //TODO ADDED CODE*/
    ...
    public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
        ...
        if (this.player.playerInteractManager.isCreative()) {
            ...
            // CraftBukkit start - Call clock event
    ...
    public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
        if (this.disconnecting) return;
        PlayerConnectionUtils.ensureMainThread(packetplayinsetcreativeslot, this, this.player.u());
        if (checkPacketLimit(packetplayinsetcreativeslot)) return;
        long start = System.currentTimeMillis();
        if (this.player.playerInteractManager.isCreative()) {
            if (packetplayinsetcreativeslot.isInvalid()) {
                RevampSpigot.getInstance().getLogger().info("{0} Invalid NBT item (" + packetplayinsetcreativeslot.getClass().getSimpleName() + ")!", this.player.getName());
                disconnect(CC.AQUA + "RevampSpigot -> Invalid packet received!");
                sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid NBT item");
                return;
            }
            boolean flag = (packetplayinsetcreativeslot.a() < 0);
            ItemStack itemstack = packetplayinsetcreativeslot.getItemStack();
            if (itemstack != null) {
                if (Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.DEBUG && itemstack.getTag() != null)
                    RevampSpigot.getInstance().getLogger().info("{0} Packet: {1} [Item: {2} | tag: {3}]", this.player.getName(), packetplayinsetcreativeslot.getClass().getSimpleName(), itemstack.getItem().getName(), itemstack.getTag().toString());
                if (itemstack.getItem() instanceof ItemWrittenBook || itemstack.getItem() instanceof ItemBookAndQuill) {
                    if (itemstack.getItem() instanceof ItemWrittenBook) {
                        if (itemstack.getTag() != null && !ItemWrittenBook.isValid(itemstack.getTag())) {
                            RevampSpigot.getInstance().getLogger().info("{0} Tried to clock in invalid item!", this.player.getName());
                            disconnect(CC.AQUA + "RevampSpigot -> Invalid packet sent!");
                            sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book item v1");
                            return;
                        }
                    } else if (itemstack.getTag() != null && !ItemBookAndQuill.isValid(itemstack.getTag())) {
                        RevampSpigot.getInstance().getLogger().info("{0} Tried to clock in invalid item!", this.player.getName());
                        disconnect(CC.AQUA + "RevampSpigot -> Invalid packet sent!");
                        sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid book item v2");
                        return;
                    }
                } else if ((itemstack.getItem() instanceof ItemFireworks || itemstack.getItem() instanceof ItemFireworksCharge) && itemstack.getTag() != null) {
                    int length = itemstack.getTag().toString().length();
                    int maxLength = RevampSpigot.getInstance().getAntiCrashFireworkNbtLimit();
                    if (length > maxLength) {
                        RevampSpigot.getInstance().getLogger().info("{0} Tried to sent invalid item in blockPlace! Item: firework | Length: {1}, max allowed: {2}", this.player.getName(), Integer.valueOf(length), Integer.valueOf(maxLength));
                        disconnect(CC.AQUA + "RevampSpigot -> Invalid item sent!");
                        sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Invalid firework data");
                        return;
                    }
                }
            }
            if (itemstack != null && itemstack.hasTag() && itemstack.getTag().hasKeyOfType("BlockEntityTag", 10)) {
                ...
                if (nbttagcompound.hasKey("x") && nbttagcompound.hasKey("y") && nbttagcompound.hasKey("z")) {
                    ...
                    if (tileentity != null) {
                        ...
                        itemstack.a("BlockEntityTag", nbttagcompound2);
                    }
                }
            }
            boolean flag2 = (packetplayinsetcreativeslot.a() >= 1 && packetplayinsetcreativeslot.a() < 36 + PlayerInventory.getHotbarSize());
            boolean flag3 = (itemstack == null || (itemstack.getItem() != null && (!invalidItems.contains(Item.getId(itemstack.getItem())) || !SpigotConfig.filterCreativeItems)));
            boolean flag4 = (itemstack == null || (itemstack.getData() >= 0 && itemstack.count <= 64 && itemstack.count > 0));
            if (flag || (flag2 && !ItemStack.matches(this.player.defaultContainer.getSlot(packetplayinsetcreativeslot.a()).getItem(), packetplayinsetcreativeslot.getItemStack()))) {
                ...
                InventoryCreativeEvent event = new InventoryCreativeEvent(craftInventoryView, type, flag ? -999 : packetplayinsetcreativeslot.a(), item);
                this.server.getPluginManager().callEvent(event);
                itemstack = CraftItemStack.asNMSCopy(event.getCursor());
                switch (event.getResult()) {
                    case ALLOW:
            ...
            if (flag2 && flag3 && flag4) {
                ...
                this.player.defaultContainer.a(this.player, true);
            } else if (flag && flag3 && flag4 && this.m < 200) {
                this.m += 20;
                EntityItem entityitem = this.player.drop(itemstack, true);
                if (entityitem != null)
                    entityitem.j();
            ...
            RevampSpigot.getInstance().getLogger().info("{0} Tried to clock in creative inventory without creative gamemode!", this.player.getName());
            disconnect(CC.AQUA + "RevampSpigot -> Invalid packet sent!");
            sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Clicked creative inv without gamemode 1");
        }
    }*/
...
/*
    ...
    public void a(PacketPlayInSetCreativeSlot packetplayinsetcreativeslot) {
        if (this.disconnecting)
            return;
        PlayerConnectionUtils.ensureMainThread(packetplayinsetcreativeslot, this, this.player.u());
        if (checkPacketLimit(packetplayinsetcreativeslot))
            return;
        long start = System.currentTimeMillis();
        if (this.player.playerInteractManager.isCreative()) {
            if (packetplayinsetcreativeslot.isInvalid()) {
            ...
            RevampSpigot.getInstance().getLogger().info("{0} Tried to click in creative inventory without creative gamemode!", this.player.getName());
            disconnect("[RevampSpigot] -> Invalid packet sent!");
            sendStaffMessage(packetplayinsetcreativeslot.getClass().getSimpleName(), System.currentTimeMillis() - start, "Clicked creative inv without gamemode 1");
        }
    }
...
*/
    ...
    public void a(PacketPlayInCustomPayload packetplayincustompayload) {
        ...
        int capacityLimit = Settings.IMP.SETTINGS.ANTICRASH.MAX_PAYLOAD_CAPACITY;
        if (capacity > capacityLimit) {
            RevampSpigot.getInstance().getLogger().info("{0} -> too big payload capacity! (" +
                    capacity + " is bigger than limit: " + capacityLimit + ")", this.player.getName());
            disconnect("[RevampSpigot] -> Invalid payload packet!");
            sendStaffMessage(packetplayincustompayload.getClass().getSimpleName(), System.currentTimeMillis() - start,
                    "Invalid payload data (" + packetplayincustompayload.a() + ")");
            return;
        }
        if (checkPacketLimit(packetplayincustompayload))
            return;
        PlayerConnectionUtils.ensureMainThread(packetplayincustompayload, this, this.player.u());
        try {
...
/*
    private void sendStaffMessage(String packet, long time, String details) {
        String permission = Settings.IMP.MESSAGES.ANTICRASH_STAFF_PERMISSION;
        String message = Settings.IMP.MESSAGES.ANTICRASH_STAFF_MESSAGE;
        message = StringUtils.replace(message, "{PLAYER}", this.player.getName());
        message = StringUtils.replace(message, "{PACKET}", packet);
        message = StringUtils.replace(message, "{TIME}", String.valueOf(time));
        message = StringUtils.replace(message, "{DETAILS}", details);
        message = StringUtils.replace(message, "%nl%", "\n");
        String adminMessage = CC.translate(message);
        Bukkit.getOnlinePlayers().parallelStream()
                .filter(player -> player.hasPermission(permission))
                .forEach(player -> player.sendMessage(adminMessage));
        Bukkit.getPluginManager().callEvent(new ExploitDetectedEvent(this.player.getName(), this.player.getBukkitEntity().getAddress().getAddress().getHostAddress(), "Invalid packet received, packet: " + packet + ", more details: " + details));
    }*/
    //TODO ADDED CODE
