...
public abstract class MobSpawnerAbstract {
    ...
    public void c() {
        ...
        boolean mobStacking = RevampSpigot.getInstance().isMobStacking();
        if (!RevampSpigot.getInstance().isCheckForNearbyPlayers() || g()) {
            BlockPosition blockposition = b();
            if ((a()).isClientSide) {
                if (RevampSpigot.getInstance().isSpawnerParticles()) {
                    ...
                    a().addParticle(EnumParticle.SMOKE_NORMAL, d1, d2, d0, 0.0D, 0.0D, 0.0D);
                    a().addParticle(EnumParticle.FLAME, d1, d2, d0, 0.0D, 0.0D, 0.0D);
                }
                ...
                for (int i = 0; i < this.spawnCount; i++) {
                    double d0, d3, d4;
                    SpawnerPreSpawnEvent event = new SpawnerPreSpawnEvent(new Location(a().getWorld(), blockposition.getX(), blockposition.getY(), blockposition.getZ()), EntityType.fromName(getMobName()));
                    ...
                    if (entity == null)
                        return;
                    entity.motY = RevampSpigot.getInstance().getInitialYVelocityBySpawner();
                    if (RevampSpigot.getInstance().isSpawnerMaxNearbyCheck()) {
                    ...
                    double newSpawnRange = this.spawnRange + 0.5D;
                    if (RevampSpigot.getInstance().isUseLessRandomSpawnPosition()) {
                        double randomDouble = (a()).random.nextDouble() - (a()).random.nextDouble();
                        d0 = blockposition.getX() + randomDouble * newSpawnRange;
                        d3 = (blockposition.getY() + (a()).random.nextInt(3) - 1);
                        d4 = blockposition.getZ() + -randomDouble * newSpawnRange;
                    } else {
                        d0 = blockposition.getX() + ((a()).random.nextDouble() - (a()).random.nextDouble()) * newSpawnRange;
                        d3 = (blockposition.getY() + (a()).random.nextInt(3) - 1);
                        d4 = blockposition.getZ() + ((a()).random.nextDouble() - (a()).random.nextDouble()) * newSpawnRange;
                    }
                    EntityInsentient entityinsentient = (entity instanceof EntityInsentient) ? (EntityInsentient)entity : null;
                    entity.setPositionRotation(d0, d3, d4, RevampSpigot.getInstance().isUseLessRandomSpawnPosition() ? 360.0F : ((a()).random.nextFloat() * 360.0F), 0.0F);
                    if (mobStacking) {
                        if (entityinsentient instanceof EntityZombie && entityinsentient.isBaby()) {
                            EntityZombie entityZombie = (EntityZombie)entityinsentient;
                            entityZombie.setBaby(false);
                        }
                        entity.isFromMobSpawner = true;
                        entity.stackSize = this.spawnCount;
                        entity.searchNear();
                        entity.updateName();
                        if (entity.dead) {
                            flag = true;
                    ...
                    if (entityinsentient == null || ((!RevampSpigot.getInstance().isLightLevelCheck() || entityinsentient.bR()) && entityinsentient.canSpawn())) {
                        a(entity, true);
                        if (RevampSpigot.getInstance().isSpawnerParticles()) {
