...
public class PacketDataSerializer extends ByteBuf {
    ...
    public void a(ItemStack itemstack) {
        if (itemstack == null || itemstack.getItem() == null) { // CraftBukkit - NPE fix itemstack.getItem()
            ...
            if (nbttagcompound != null) {
                if ((itemstack.getItem() == Items.WRITTEN_BOOK || itemstack.getItem() == Items.WRITABLE_BOOK) && nbttagcompound.hasKeyOfType("pages", 9)) {
                    ...
                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        ...
                        newList.add(new NBTTagString(IChatBaseComponent.ChatSerializer.a(root)));
                    }
    ...
    public ItemStack i() throws IOException {
        ...
        if (short0 >= 0) {
            ...
            if (tag != null) {
                if ((itemstack.getItem() == Items.WRITTEN_BOOK || itemstack.getItem() == Items.WRITABLE_BOOK) && tag.hasKeyOfType("pages", 9)) {
                    NBTTagList nbttaglist = tag.getList("pages", 8);
                    NBTTagList newList = new NBTTagList();
                    for (int i = 0; i < nbttaglist.size(); ++i) {
                        IChatBaseComponent s = IChatBaseComponent.ChatSerializer.a(nbttaglist.getString(i));
                        String newString = SpigotComponentReverter.toLegacy(s);
                        newList.add(new NBTTagString(newString));
                    }
                    tag.set("pages", newList);
                }
