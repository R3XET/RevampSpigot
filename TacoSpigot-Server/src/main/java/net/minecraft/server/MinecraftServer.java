...
public abstract class MinecraftServer implements Runnable, ICommandListener, IAsyncTaskHandler, IMojangStatistics {
    ...
    public AffinityLock affinitylock = null;
    // CraftBukkit end
    ...
    public MinecraftServer(OptionSet options, Proxy proxy, File file1) {
        ...
        try (AffinityLock al = AffinityLock.acquireLock()) {
        	this.affinitylock = al;
        	LOGGER.info("Main is now locked.");
    ...
    public void stop() throws ExceptionWorldConflict { // CraftBukkit - added throws
        ...
        if (!this.N) {
            ...
            //TODO ADDED CODE
            if (Settings.IMP.SETTINGS.KICK_PLAYERS_ON_STOP) {
                for (EntityPlayer player : (getPlayerList()).players)
                    player.playerConnection.disconnect(Settings.IMP.MESSAGES.SHUTDOWN_MESSAGE);
            }
            //TODO ADDED CODE
    ...
    private static long SEC_IN_NANO = 1000000000;
...
/*
    ...
    public void run() {
        try {
            TPS = Settings.IMP.SETTINGS.DEVELOPER_SETTINGS.TPS;
            TICK_TIME = SEC_IN_NANO / TPS;
            MAX_CATCHUP_BUFFER = TICK_TIME * TPS * 60L;

           AffinityLock al2 = affinitylock.acquireLock(AffinityStrategies.SAME_SOCKET,
            ...
            if (this.init()) {
                ...
                // Toothless start - Tick loop revamp
                // Spigot start
                // PaperSpigot start - Further improve tick loop
    ...
    public void run() {
        try {
            if (init()) {
                ...
                long start = System.nanoTime(), lastTick = start - 50000000L, catchupTime = 0L, tickSection = start;
                while (this.isRunning) {
