...
import eu.revamp.spigot.events.plants.CactusGrowEvent;
...
public class CraftEventFactory {
    ...
    public static PlayerInteractEvent callPlayerInteractEvent(EntityHuman who, Action action, BlockPosition position, EnumDirection direction, ItemStack itemstack, boolean cancelledBlock) {
        ...
        //TODO ADDED CODE
        if (!event.isCancelled() && (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK) && itemInHand != null) {
            ...
            if (name.contains("_HELMET") || name.contains("_CHESTPLATE") || name.contains("_LEGGINGS") || name.contains("_BOOTS")) {
                ArmorEvents.check(event.getPlayer());
            }
        }
    ...
    public static PlayerDeathEvent callPlayerDeathEvent(EntityPlayer victim, List<org.bukkit.inventory.ItemStack> drops, String deathMessage, boolean keepInventory) {
        ...
        //TODO ADDED CODE
        ArmorEvents.check(event.getEntity());
        //TODO ADDED CODE
        return event;
    }
    ...
    public static void handleBlockGrowEvent(World world, int x, int y, int z, net.minecraft.server.Block type, int data) {
        ...
        if (block.getType() == Material.CACTUS) {
            CactusGrowEvent event = new CactusGrowEvent(block, state);
            Bukkit.getPluginManager().callEvent(event);
            if (!event.isCancelled()) {
                state.update(true);
            }
        ...
        BlockGrowEvent event2 = new BlockGrowEvent(block, state);
